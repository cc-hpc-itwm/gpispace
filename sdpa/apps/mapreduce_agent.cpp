...
int main (int argc, char **argv)
{
  ...
  desc.add_options()
     ...
     ("capabilities,c", po::value<vector<string> >(&arrCapabilities)->multitoken(), "Agent's capabilities")
     ...
     ("type,t", po::value<string>(&agentType)->default_value(""), "Possible types are: mapper, reducer or collector")
     ;
  ...
  case FILE_DEF:
          LOG( INFO
             , "Backup folder not specified! No backup file will be created!");
          bDoBackup = false;
      break;
  ...
  try
  {
    ...
    if( agentType == "mapper")
      ptrAgent = sdpa::daemon::AgentFactory<MapperWorkflowEngine<WordCountMapper> >::create(  agentName,
                                                                                              agentUrl,
                                                                                              listMasterInfo,
                                                                                              MAX_CAP,
                                                                                              bCanExecuteJobs,
    ...
    else if( agentType == "reducer")
      ptrAgent = sdpa::daemon::AgentFactory<ReducerWorkflowEngine<WordCountMapper, WordCountReducer> >::create(  agentName,
                                                                                              agentUrl,
                                                                                              listMasterInfo,
                                                                                              MAX_CAP,
                                                                                              bCanExecuteJobs,
      ...
      ptrAgent = sdpa::daemon::AgentFactory<CollectorWorkflowEngine<WordCountMapper, WordCountReducer> >::create(  agentName,
                                                                                                                    agentUrl,
                                                                                                                    listMasterInfo,
                                                                                                                    MAX_CAP,
                                                                                                                    bCanExecuteJobs,
    ...
    {
      LOG(FATAL, "No valid type specified for the agent. Please, use the option -t to specify they type (mapper, reducer or collector)");
      return -1;
    }


    // add now the capabilities:
    BOOST_FOREACH(string strCpb, arrCapabilities)
    {
      ptrAgent->addCapability( sdpa::capability_t(strCpb, "node", ptrAgent->name()));
    }
