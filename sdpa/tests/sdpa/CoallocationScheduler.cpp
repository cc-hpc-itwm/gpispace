...
BOOST_AUTO_TEST_CASE (scheduling_with_preassignment)
{
  const unsigned int n_req_workers (1);

  const std::vector<sdpa::job_id_t>
    job_ids {"job_0", "job_1", "job_2", "job_3", "job_4", "job_5", "job_6"};

  const std::vector<sdpa::worker_id_t>
    worker_ids {"worker_0", "worker_1"};

  const std::function<double (std::string const&)>
    test_transfer_cost ( [](const std::string& worker) -> double
                         {
  ...
  sdpa::daemon::CoallocationScheduler
    _scheduler ( [&job_ids, &worker_ids] (const sdpa::worker_id_list_t& workers, const sdpa::job_id_t& job_id)
                 {
                   std::vector<sdpa::job_id_t>::const_iterator it (std::find (job_ids.begin(), job_ids.end(), job_id));
                   BOOST_REQUIRE (it != job_ids.end());
                   const unsigned int index (it - job_ids.begin());
                   // each third job should be assigned to the more costly worker, if the other one
                   // was assigned enough jobs to execute (the total cost of a job to execute
                   // on worker_0 is 2.0 and 5.0 on worker_1)
                   BOOST_REQUIRE ( (((index + 1) % 3 != 0) && (workers.front() == worker_ids[0]))
                                || (((index + 1) % 3 == 0) && (workers.front() == worker_ids[1]))
                                 );
                 }
               , [n_req_workers, &test_transfer_cost] (const sdpa::job_id_t&)
                 {
  ...
  _scheduler.enqueueJob (job_ids[0]);
  _scheduler.enqueueJob (job_ids[1]);
  _scheduler.enqueueJob (job_ids[2]);
  _scheduler.enqueueJob (job_ids[3]);
  _scheduler.enqueueJob (job_ids[4]);
  _scheduler.enqueueJob (job_ids[5]);
