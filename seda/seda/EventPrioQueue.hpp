...
#ifndef SEDA_EVENT_PRIO_QUEUE_HPP
#define SEDA_EVENT_PRIO_QUEUE_HPP 1
...
struct PrioQueueEventComparer
{
	bool operator()(const seda::IEvent::Ptr& lhs, const seda::IEvent::Ptr& rhs) const
	{
...
namespace seda {
    class EventPrioQueue : public IEventQueue {
        public:
            typedef seda::shared_ptr< EventPrioQueue > Ptr;

            explicit
                EventPrioQueue(const std::string& a_name, std::size_t a_maxQueueSize=SEDA_MAX_QUEUE_SIZE)
                : SEDA_INIT_LOGGER("seda.queue."+a_name),
                _name(a_name),
                _maxQueueSize(a_maxQueueSize) {}
            virtual ~EventPrioQueue() {}
        ...
        protected:
            ...
            std::priority_queue<IEvent::Ptr, std::vector<IEvent::Ptr>, PrioQueueEventComparer > _list;
