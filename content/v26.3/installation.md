...
Before updating an existing installation or rebuilding client code,
read `share/doc/migration_v24.12_to_v26.3.md`.

The most relevant installation changes in 26.3 are:

* GPI-Space CMake cache variables are now prefixed with `GSPC_`.
  The old names are still accepted with deprecation warnings, but they
  are planned to be removed by the end of 2026.
* Libraries are now installed using `CMAKE_INSTALL_LIBDIR` instead of a
  hard-coded `lib/` directory. On some systems this means runtime
  libraries will end up in `lib64/`.
* Bundling metadata was updated for renamed runtime libraries such as
  `libgspc_rpc` and `libgspc_util.so`.
* The optional expression shell can be enabled with
  `-DGSPC_WITH_EXPRESSION_SHELL=ON` and requires `libreadline`.
...
* [libreadline](https://tiswww.case.edu/php/chet/readline/rltop.html)
  * Only required if GPI-Space is built with
...
empty directory on a shared filesystem. The old
`SHARED_DIRECTORY_FOR_TESTS` name is still accepted with a deprecation
warning. The tests can then be run with
...
#build_tests="-DBUILD_TESTING=on -DGSPC_SHARED_DIRECTORY_FOR_TESTS=<shared-directory>"
...
If your site uses a `lib64/` layout, set `CMAKE_INSTALL_LIBDIR=lib64`
instead.
...
The optional `gspc-expression-shell` utility evaluates expressions
interactively or in scripts. It is disabled by default and requires
`libreadline`.

| Spack | CMake |
| - | - |
| n/a | `-D GSPC_WITH_EXPRESSION_SHELL=[ON|OFF]` |
