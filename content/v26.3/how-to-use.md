...
version: 26.3
permalink: /v26.3/how-to-use
---

This document provides a step-by-step guide on how to create a first
GPI-Space application.

## Read this before starting with v26.3

Release 26.3 contains a major public API migration. If you are porting an
existing application from 24.12 or earlier, read the migration guide first:
`share/doc/migration_v24.12_to_v26.3.md`.

The most important changes for users of this tutorial are:

* Public headers are now installed under `include/gspc/`. For example,
  `#include <drts/drts.hpp>` becomes `#include <gspc/drts/drts.hpp>` and
  `#include <util-generic/print_exception.hpp>` becomes
  `#include <gspc/util/print_exception.hpp>`.
* Public namespaces are consolidated under `gspc::`. For example,
  `fhg::util` becomes `gspc::util`, `we::type` becomes `gspc::we::type`,
  and `pnet::type::value` becomes `gspc::pnet::type::value`.
* Workflow XML files must update `<cinclude>` paths to the new `gspc/...`
  layout, and workflow modules that linked with `-lwe-dev` now need
  `-lgspc_we`.
* Several low-level CMake targets were renamed. In particular,
  `Util::Generic` is now `gspc_util`, `Util::RPC` is now `gspc_rpc`, and
  `GPISpacePrivate::we-dev` is now `GPISpacePrivate::gspc_we`.

The examples below focus on the workflow concepts and application structure.
When implementing them with v26.3, use the renamed headers, namespaces,
and linker targets from the migration guide.

## New workflow features in v26.3

Release 26.3 also adds several workflow features that are useful when
designing new applications:

* Generator places can automatically produce unique values for supported
  token types. See `share/doc/generator_places.md`.
* The new `shared` type enables reference-counted cleanup of workflow
  values. See `share/doc/reference_counted_tokens.md`.
* The expression language now supports arbitrary-precision `bigint`
  values.
* The optional `gspc-expression-shell` utility can be enabled at build
  time for interactive expression evaluation.
...
namespace aggregate_sum
{
  ...
  WorkflowResult execute (Parameters parameters, Workflow const& workflow)
  {
    auto const aggregate_sum_installation_path
      (gspc::util::executable_path().parent_path().parent_path());
