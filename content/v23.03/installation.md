...
- libssh2: `Libssh2_ROOT`. By default CMake's new config format will be
  automatically preferred if existing.
  Using config format installations can be disabled with the CMake option
  `-D Libssh2_NO_CMAKE=ON` or the environment variable `Libssh2_NO_CMAKE=ON`.
