// Copyright (C) 2013-2024,2026 Fraunhofer ITWM
...
#define ENTRY(name_, color_, level_, identifier_)      \
      { gspc::logging::legacy::category_level_ ## name_   \
...
#undef ENTRY
  ...
  {
    using put_time = gspc::util::ostream::put_time<typename Timepoint::clock>;
    return QString::fromStdString (put_time (timepoint).string());
  }
}

namespace gspc::monitor::detail
{
  formatted_log_event::formatted_log_event (gspc::logging::message raw)
  ...
  std::vector<gspc::logging::message> log_table_model::data() const
  {
    QMutexLocker lock (&_mutex_data);

    std::vector<gspc::logging::message> result;
  ...
  void log_table_model::add (gspc::logging::message message)
  {
...
void log_monitor::append_log_event (gspc::logging::message const& message)
{
  if (message._category == gspc::scheduler::daemon::gantt_log_category)
  {
    return;
  }
...
void log_monitor::save ()
{
  ...
  try
  {
    ...
    {
      ofs << "["
          << gspc::util::ostream::put_time<decltype (message._timestamp)::clock>
               (message._timestamp)
          << "] "
          << message._hostname
          << ": "
