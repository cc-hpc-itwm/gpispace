    ...
    namespace gspc::xml::parse::error
    {
        ...
        , type::port_type const& port
          ...
          : generic
            { fmt::format
              ...
              , port.name()
              , port.type()
              , gspc::pnet::type::signature::show (port.signature())
              , ::gspc::we::response_description_requirements()
              )
            , response.position_of_definition()
            }
      {}
      ...
      output_not_connected::output_not_connected
       ...
       , type::port_type const& port
         ...
         : generic { fmt::format ( "output port {} is not connected"
                                   " in transition {} at {}"
                                 , port.name()
                                 , transition.name()
                                 , port.position_of_definition()
      ...
      contradicting_shared_attributes::contradicting_shared_attributes
       ( type::transition_type const& transition
       )
         : generic { fmt::format ( "contradicting shared tracking attributes:"
                                   " passthrough_shared cannot be combined with"
                                   " produce_shared or consume_shared"
                                   " in transition {} at {}"
                                 , transition.name()
                                 , transition.position_of_definition()
                                 )
                   }
        {}
      ...
      connect_type_error::connect_type_error
        ...
        , type::place_type const& place
          ...
          : generic { fmt::format ( "type-error: connect-{}"
                                  ...
                                  , place.name()
                                  , place.type()
                                  , gspc::pnet::type::signature::show (place.signature())
      ...
      namespace
      {
          ...
          (MemoryBuffers const& memory_buffers)
        {
          return gspc::util::print_container
            ( "{", ", ", "}", memory_buffers
            , gspc::util::ostream::callback::generic< type::memory_buffer_type
                                                   , util::position_type
                                                   >
      ...
      {
        template<typename T>
        gspc::util::join_reference<std::list<T>, std::string>
          print_positions (std::string const& prefix, std::list<T> const& list)
        {
          return gspc::util::print_container
              ( prefix + ": (", ", ", ")", list
              , gspc::util::ostream::callback::select<T, util::position_type>
          ...
          (type::function_type const& function)
        {
          ...
          {
            oss << sep << print_positions<gspc::xml::parse::type::memory_get>
          ...
          {
            oss << sep << print_positions<gspc::xml::parse::type::memory_put>
          ...
          {
            oss << sep << print_positions<gspc::xml::parse::type::memory_getput>
      ...
      shared_place_does_not_exist::shared_place_does_not_exist
        ( type::place_type const& place
        , std::string const& cleanup_place_name
        )
          : generic
            { fmt::format
              ( "shared cleanup place '{}' does not exist,"
                " referenced by type '{}' for place '{}' at {}"
              , cleanup_place_name
              , place.type()
              , place.name()
              , place.position_of_definition()
      ...
      shared_place_not_marked_as_sink::shared_place_not_marked_as_sink
        ( type::place_type const& place
        , std::string const& cleanup_place_name
        , type::place_type const& cleanup_place
          ...
          : generic
            { fmt::format
              ( "shared cleanup place '{}' at {} must have attribute"
                " shared_sink=\"true\", referenced by type '{}'"
                " for place '{}' at {}"
              , cleanup_place_name
              , cleanup_place.position_of_definition()
              , place.type()
              , place.name()
              , place.position_of_definition()
              )
      ...
      generator_place_with_tokens::generator_place_with_tokens
        (type::place_type const& place)
          : generic
            { fmt::format
              ( "generator place '{}' at {} must not have <token> elements"
              , place.name()
              , place.position_of_definition()
              )
            }
      ...
      generator_place_with_put_token::generator_place_with_put_token
        (type::place_type const& place)
          : generic
            { fmt::format
              ( "generator place '{}' at {} must not have"
      ...
      generator_place_with_shared_sink::generator_place_with_shared_sink
        (type::place_type const& place)
          : generic
            { fmt::format
              ( "generator place '{}' at {} must not have"
      ...
      generator_place_invalid_type::generator_place_invalid_type
        (type::place_type const& place)
          : generic
            { fmt::format
              ( "generator place '{}' at {} has invalid type '{}',"
      ...
      generator_place_with_read_connection::generator_place_with_read_connection
        ( type::place_type const& place
        , type::transition_type const& transition
        )
          : generic
            { fmt::format
              ( "generator place '{}' at {} must not have read connections,"
                " but transition '{}' at {} has a read connection to it"
              , place.name()
              , place.position_of_definition()
              , transition.name()
              , transition.position_of_definition()
      ...
      generator_place_with_incoming_connection
        ...
        , type::transition_type const& transition
          ...
          : generic
            { fmt::format
              ( "generator place '{}' at {} must not have"
                " incoming connections, but transition"
                " '{}' at {} has a connection to it"
              , place.name()
              , place.position_of_definition()
              , transition.name()
              , transition.position_of_definition()
      ...
      namespace
      {
       ...
       {
         return !_list.empty() ? gspc::util::print_container ( _prefix + " ('"
                                                            , "', '"
                                                            , "')"
                                                            , _list
                                                            ).string()
