// Copyright (C) 2012-2026 Fraunhofer ITWM
...
#include <gspc/we/type/Expression.fwd.hpp>
...
#include <gspc/we/type/signature/cpp.hpp>
#include <gspc/we/type/signature/is_literal.hpp>
#include <gspc/we/type/signature/names.hpp>
...
#include <optional>
    ...
    namespace gspc::xml::parse::type
    {
      function_type::function_type
        ...
        , fhgpnet::util::unique<memory_buffer_type> const& memory_buffers
        ...
        , _properties (properties)
      {
        for (port_type const& port : _ports)
        {
          if (!port.is_tunnel())
          {
            ::std::optional<std::reference_wrapper<port_type const>> const other
            ...
            if (other && port.type() != other->get().type())
            {
              throw error::port_type_mismatch (port, other->get());
            }
          }

          ::std::optional<std::reference_wrapper<memory_buffer_type const>> const memory_buffer
        ...
        {
          {
            ::std::optional<std::reference_wrapper<port_type const>> const port_in
          ...
          {
            ::std::optional<std::reference_wrapper<port_type const>> const port_out
        ...
        if (!gspc::util::cxx17::holds_alternative<module_type> (_content))
        {
      ...
      {
        return gspc::util::cxx17::holds_alternative<net_type> (content());
      }
      ...
      std::optional<std::string> const& function_type::name() const
      {
        return _name;
      }
      ...
      std::optional<std::reference_wrapper<port_type const>>
      function_type::get_port_in (std::string const& name) const
      {
        return ports().get ({name, ::gspc::we::type::port::direction::In{}});
      }

      std::optional<std::reference_wrapper<port_type const>>
      function_type::get_port_out (std::string const& name) const
      {
        return ports().get ({name, ::gspc::we::type::port::direction::Out{}});
      }
      ...
      {
        return ports().has ({name, ::gspc::we::type::port::direction::Tunnel{}});
      }
      ...
      std::string conditions_type::flatten() const
      {
        return empty()
          ? "true"
          : gspc::util::join (begin(), end(), " && ", "(", ")").string();
      }
      ...
      class function_synthesize
        : public ::boost::static_visitor<::gspc::we::type::Transition>
      {
      private:
        ...
        std::unordered_map<std::string, ::gspc::we::port_id_type>& _port_id_in;
        std::unordered_map<std::string, ::gspc::we::port_id_type>& _port_id_out;
        ::gspc::we::priority_type _priority;
        fhgpnet::util::unique<place_map_type> const& _place_map;
        std::unordered_map<::gspc::we::port_id_type, std::string>& _real_place_names;
        ::gspc::we::type::track_shared _track_shared;

        using pid_of_place_type =
          std::unordered_map<std::string, ::gspc::we::place_id_type>;

        ::gspc::we::port_id_type add_port
          (::gspc::we::type::Transition& transition, ::gspc::we::type::Port const& port) const
        {
          ::gspc::we::port_id_type const port_id (transition.add_port (port));
        ...
        void add_ports ( ::gspc::we::type::Transition & trans
                       , function_type::ports_type const& ports
                       ) const
        {
          for (port_type const& port : ports)
          {
            add_port (trans, ::gspc::we::type::Port ( port.name()
                                              , port.direction()
                                              , port.signature()
                                              , port.properties()
                                              )
        ...
        void add_requirements (::gspc::we::type::Transition& trans) const
        {
          ...
          {
            trans.add_requirement (::gspc::we::type::Requirement {req});
          }
        }
        ...
        ::gspc::we::type::Expression condition() const
        {
          const std::string cond ((fun.conditions() + _conditions).flatten());

          gspc::we::expr::parse::parser parsed_condition
            (util::we_parse (cond, "condition", "function", name(), fun.position_of_definition().path()));

          return ::gspc::we::type::Expression (cond, parsed_condition);
        }

        ::gspc::we::type::ModuleCall create_we_module (module_type const& mod) const
        {
          ...
          std::list<::gspc::we::type::memory_transfer> memory_gets;
          std::list<::gspc::we::type::memory_transfer> memory_puts;
          ...
          for (memory_get const& mg : fun.memory_gets())
          {
            memory_gets.emplace_back
              ...
              , mg.local()
              , std::nullopt
              , mg.allow_empty_ranges().value_or (false)
              );
          }
          for (memory_put const& mp : fun.memory_puts())
          {
            auto not_mod = mp.not_modified_in_module_call();
            memory_puts.emplace_back
              ( mp.global()
              , mp.local()
              , not_mod
              , mp.allow_empty_ranges().value_or (false)
              );
          }
          for (memory_getput const& mgp : fun.memory_getputs())
          {
            memory_gets.emplace_back
              ...
              , mgp.local()
              , std::nullopt
              , mgp.allow_empty_ranges().value_or (false)
              );
            auto not_mod = mgp.not_modified_in_module_call();
            memory_puts.emplace_back
              ( mgp.global()
              , mgp.local()
              , not_mod
          ...
          return ::gspc::we::type::ModuleCall
            ( mod.name()
            , mod.function()
      ...
      public:
        function_synthesize
          ...
          , conditions_type const& conditions
          , ::gspc::we::type::property::type const& trans_properties
          ...
          , ::gspc::we::priority_type priority
          , fhgpnet::util::unique<place_map_type> const& place_map
          , std::unordered_map<::gspc::we::port_id_type, std::string>& real_place_names
          , ::gspc::we::type::track_shared track_shared_flags
          )
          : _name (name)
          , state (_state)
          , fun (_fun)
          ...
          , _track_shared (track_shared_flags)
        {
        ...
        operator () (expression_type const& e) const
        {
          ...
          const gspc::we::expr::parse::parser parsed_expression
            (util::we_parse (expr, "expression", "function", name(), fun.position_of_definition().path()));

          ::gspc::we::type::Transition trans
            ( name()
            , ::gspc::we::type::Expression (expr, parsed_expression)
            , condition()
            , _properties
            , _priority
        ...
        ::gspc::we::type::Transition operator () (module_type const& mod) const
        {
          ::gspc::we::type::Transition trans
            ( name()
        ...
        ::gspc::we::type::Transition operator () (net_type const& net) const
        {
          ...
          ::gspc::we::type::property::type properties (_properties);
        ...
        ::gspc::we::type::Transition operator () (multi_module_type const& multi_mod) const
        {
          ::gspc::we::type::MultiModuleCall multi_modules;
      ...
      ::gspc::we::type::Transition function_type::synthesize
        ( std::string const& name
        , state::type const& state
        , std::unordered_map<std::string, ::gspc::we::port_id_type>& port_id_in
        , std::unordered_map<std::string, ::gspc::we::port_id_type>& port_id_out
      ...
      void function_type::specialize (state::type & state)
      {
        ...
        const gspc::xml::parse::structure_type_util::set_type known_empty;
      ...
      ::gspc::we::type::property::type const& function_type::properties() const
      {
        return _properties;
      }
                      ...
                      )
      {
        ...
        for (fun_info_map::value_type const& mod : m)
        {
          ...
          stream << gspc::util::cpp::include ("gspc/we/loader/macros.hpp");
          ...
          stream << gspc::util::cpp::block::open (indent);
          for (fun_info_type const& fun : funs)
          {
            stream << indent
                   << "WE_REGISTER_FUN_AS ("
          ...
          stream << gspc::util::cpp::block::close (indent) << std::endl;
                       ...
                       )
      {
        ...
        if (not std::filesystem::is_directory (prefix))
        {
      ...
      namespace
      {
        ...
        namespace
        {
          class include : public gspc::util::ostream::modifier
          {
          public:
              ...
              , _suffix (suffix)
            {
              ...
              _inc[value::CONTROL()].insert ("gspc/we/type/literal/control.hpp");
              _inc[value::STRING()].insert ("string");
              _inc[value::BITSET()].insert ("gspc/we/type/bitsetofint.hpp");
              _inc[value::BYTEARRAY()].insert ("gspc/we/type/bytearray.hpp");
              _inc[value::SHARED()].insert ("gspc/we/type/shared.hpp");
              _inc[value::LIST()].insert ("list");
              _inc[value::LIST()].insert ("gspc/we/type/value.hpp");
              _inc[value::SET()].insert ("set");
              _inc[value::SET()].insert ("gspc/we/type/value.hpp");
              _inc[value::MAP()].insert ("map");
              _inc[value::MAP()].insert ("gspc/we/type/value.hpp");
            }

            std::ostream& operator() (std::ostream& os) const override
            {
              if (!gspc::pnet::type::signature::is_literal (_tname))
              {
                os << gspc::util::cpp::include ("pnetc/type/" + _tname + _suffix);
              }
              else
              {
                auto
                ...
                else
                {
                  ...
                  //
                  if (::gspc::we::type::shared::cleanup_place (_tname))
                  {
                    os << gspc::util::cpp::include ("gspc/we/type/shared.hpp");
                  }
                }
              }
        ...
        class mk_type : public gspc::util::ostream::modifier
        {
        public:
          ...
          std::ostream& operator() (std::ostream& os) const override
          {
            ...
            {
              os << gspc::pnet::type::signature::complete (_type);
            }
            else
            {
        ...
        class mk_get : public gspc::util::ostream::modifier
        {
        public:
          mk_get ( gspc::util::indenter& indent
          ...
          std::ostream& operator() (std::ostream& os) const override
          {
            ...
            if (gspc::pnet::type::signature::is_literal (_port.type))
            {
              using gspc::pnet::type::signature::complete;
        ...
        std::string mk_value (port_with_type const& port)
        {
          ...
          if (gspc::pnet::type::signature::is_literal (port.type))
            {
              os << port.name;
            }
          else
        ...
        mod_signature ( Stream& s
                      , gspc::util::indenter& indent
                      , std::optional<port_with_type> const& port_return
                      ...
                      )
        {
          ...
          for (std::string const& memory_buffer_name : mod.memory_buffer_arg())
          {
            ...
            if (memory_buffer.read_only().value_or (false))
            {
              s << " const";
            }
        ...
        {
          namespace block = gspc::util::cpp::block;
          namespace ns = gspc::util::cpp::ns;
          namespace cpp = gspc::util::cpp;
          using gspc::util::deeper;
          ...
          s << deeper (indent) << ", gspc::we::expr::eval::context const&";
          if (!ports_const.empty() || !ports_mutable.empty())
          {
            s << "_pnetc_input";
          }
          s << deeper (indent) << ", gspc::we::expr::eval::context&";
          ...
          if (port_return)
          {
            ...
            if (!gspc::pnet::type::signature::is_literal ((*port_return).type))
            {
      ...
      namespace
      {
        ...
        public:
          ...
          {
            namespace cpp_util = ::gspc::util::cpp;
            ...
            std::optional<port_with_type> port_return;
            types_type types;
...

            {
              ...
              if (mod.pass_context ())
              {
                stream << cpp_util::include ("gspc/drts/worker/context_fwd.hpp");
              }
              for (std::string const& tname : types)
              {
...

            {
                ...
                , [] (std::string const& l, std::string const& r)
                  {
                    std::list<std::string> const
                      ls (gspc::util::split<std::string, std::string> (l, '\n'));
                    std::list<std::string> const
                      rs (gspc::util::split<std::string, std::string> (r, '\n'));
                    ...
                    {
                      if (! ((  gspc::util::starts_with ("#line", *pos_l)
                             && gspc::util::starts_with ("#line", *pos_r)
                             )
                            || *pos_l == *pos_r
                            )
                         )
              ...
              if (mod.pass_context ())
              {
                stream << cpp_util::include ("gspc/drts/worker/context.hpp");
              }
      ...
      namespace
      {
                    ...
                    )
        {
          for (structure_type const& structure : structs)
          {
            ...
            const std::filesystem::path prefix (state.path_to_cpp());

            {
              ...
              const gspc::pnet::type::signature::signature_type sig
                (structure.signature());

              stream << "// " << gspc::pnet::type::signature::show (sig) << std::endl;

              stream << gspc::util::cpp::include_guard::open
                ("PNETC_TYPE_" + structure.name());

              const std::unordered_set<std::string> names
                (gspc::pnet::type::signature::names (sig));
              ...
              stream << gspc::pnet::type::signature::cpp::header_signature (sig)
                     << std::endl;

              stream << gspc::util::cpp::include_guard::close();
            }

            {
              ...
              stream << gspc::util::cpp::include ( "pnetc/type/"
                                                + structure.name() + ".hpp"
                                                );

              for (std::string const& tname : gspc::pnet::type::signature::names (sig))
              {
                stream << include (tname, "/op.hpp");
              }

              stream << gspc::pnet::type::signature::cpp::header_op_signature (sig)
                     << std::endl;
...

            {
              ...
              stream << gspc::pnet::type::signature::cpp::impl_signature (sig)
                     << std::endl;
            }
          }
        }
      ...
      namespace dump
      {
        void dump ( ::gspc::util::xml::xmlstream &
                  , function_type const&
                  );
        ...
        namespace
        {
          ...
          public:
            function_dump_visitor (::gspc::util::xml::xmlstream & _s)
              : s (_s)
            {}
                  ...
                  )
        {
          ...
          ::gspc::we::type::property::dump::dump (s, f.properties());

          dumps (s, f.structs.begin(), f.structs.end());

          gspc::xml::parse::type::dump::dump (s, f.requirements);
...
namespace std
{
  size_t hash<gspc::xml::parse::type::fun_info_type>::operator()
    (gspc::xml::parse::type::fun_info_type const& fun) const
  {
