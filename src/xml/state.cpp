...
#include <optional>
    ...
    namespace gspc::xml::parse::state
    {
      template<typename W>
      ...
      std::filesystem::path type::expand (std::string const& file) const
      {
        const std::filesystem::path absolute (file);
        ...
        {
          const std::filesystem::path from_actual_file
            (file_in_progress().parent_path() / file);

          if (std::filesystem::exists (from_actual_file))
          {
          ...
          else
          {
            for (std::string const& search_path : _search_path)
            {
              ...
              const std::filesystem::path pre (search_path);
              const std::filesystem::path path (pre / file);
      ...
      const ::gspc::xml::parse::type::requirements_type& type::requirements() const
      {
        return _requirements;
      }

      void type::set_requirement
        (const ::gspc::xml::parse::type::require_key_type& key)
      {
        _requirements.set (key);
      }

      ::gspc::we::type::property::path_type& type::prop_path()
      {
        return _prop_path;
      }

      void
      type::interpret_property ( ::gspc::we::type::property::path_type const& path
                               , ::gspc::we::type::property::value_type const& value
                               )
      {
        auto pos (path.begin());
        ::gspc::we::type::property::path_type::const_iterator const end (path.end());
        ...
        if (pos != end && *pos == "pnetc")
        {
          ...
          if (pos != end && *pos == "search_path")
          {
            ...
            const std::filesystem::path absolute (value_str);
      ...
      void type::set_input (std::filesystem::path const& path)
      {
      ...
      {
        set_input (std::filesystem::path (file));
      }

      std::filesystem::path type::file_in_progress() const
      {
        return (_in_progress.empty())
          ? std::filesystem::path("<stdin>")
          : _in_progress.back()
          ;
      }
      ...
      std::set<std::filesystem::path> const& type::dependencies() const
      {
...
#undef WARN
      ...
      void
      type::check_for_include_loop (std::filesystem::path const& path) const
      {
...
#undef STRINGVAL
      ...
      std::filesystem::path type::strip_path_prefix
        (std::filesystem::path const& path) const
      {
        auto const try_strip
          ([&path] (std::filesystem::path const& to_strip)
           -> std::optional<std::filesystem::path>
           {
             std::filesystem::path::const_iterator
               pos_path (path.begin());
             std::filesystem::path::const_iterator
             ...
             {
               //! \note return std::filesystem::path {pos_path,
        ...
        for (auto to_strip : _path_prefixes_to_strip)
        {
          std::optional<std::filesystem::path> const stripped
            (try_strip (to_strip));
