...
extern "C" {
  ...
  */
  ...
  //! Get the total number of nodes and generate hostlist from machine file.
  /*!
  ...
  */
  ...
  //! Get the NUMA socket of the calling process
  /*!
    \param argc The argc of the process
    \param argv The argv of the process

    \return an int with the local socket number.
  */

  int getNumaSocketGPI(int argc,char *argv[]);


  //!Enable GPI to run on different NUMA sockets per node.
  /*!

    \return an int where 0 is success and -1 is failure.

    The current implementation allows GPI to run on two different NUMA sockets per node.
    Some memory areas will be initialized and mapped to the numa node of the thread calling
    startGPI. The memory region accessible thru getDmaMemPtrGPI will not be initialized and linux
    should allocate pages on the numa node on which the referencing thread is currently running (COW).   
  */

  int enableMultiSocketGPI();


  //! Enable logging for all GPI processes in a multi-socket configuration.
  /*!

    Stdout of all GPI worker processes in a multi-socket configuration will be redirected 
    to the logging network. The gpi_logger.exe utility can be used to display the output stream. 
  */

  void enableMultiSocketLoggingGPI();


  //! Disable port checks for GPI processes in a multi-socket configuration.
  /*!

    This function enables a GPI process in a multi-socket configuration to run a on single computer
    without any connectivity to other nodes or switches.
   
  */

  int disablePortCheckGPI();
  ...
  */
  ...
  /*!
    ...
    \return an int where 1 is success, 0 is worker and -1 is failure.
  */
  ...
  */
  ...
  /*!
    ...
    \return an int where 0 is success, -1 is failure and -42 is timeout.
  ...
  */
  ...
  /*!
    ...
    \param gpiMemSize The memory space allocated for GPI (bytes)
  ...
  */
  int startGPI(int argc,char *argv[],const char *cmdline,const unsigned long gpiMemSize_in_bytes);
