...
{
  BOOST_REQUIRE_EQUAL (fhg::util::cxx14::make_unique<multiple_ctors>()->invoked, default_ctor);
  BOOST_REQUIRE_EQUAL (fhg::util::cxx14::make_unique<multiple_ctors> (int())->invoked, int_ctor);
  BOOST_REQUIRE_EQUAL
    (fhg::util::cxx14::make_unique<multiple_ctors> (std::size_t())->invoked, size_t_ctor);
  BOOST_REQUIRE_EQUAL
    ( fhg::util::cxx14::make_unique<multiple_ctors> (std::size_t(), std::size_t())->invoked
    , two_size_t_ctor
    );
}
...
{
  BOOST_REQUIRE_EQUAL
    (fhg::util::cxx14::make_unique<multiple_ctors> (std::size_t (1))->invoked, size_t_ctor);
  BOOST_REQUIRE_EQUAL
    (fhg::util::cxx14::make_unique<multiple_ctors[]> (std::size_t (1))[0].invoked, default_ctor);
}
...
{
  fhg::util::cxx14::make_unique<no_default_ctor> (1);
}
...
{
  fhg::util::cxx14::make_unique<no_move_or_copy>();
}
...
{
  fhg::util::cxx14::make_unique<check_forwarding<std::string&&>> (std::string());

  int reference{};
  fhg::util::cxx14::make_unique<check_forwarding<int&>> (reference);

  int const const_reference{};
  fhg::util::cxx14::make_unique<check_forwarding<int const&>> (const_reference);
}
