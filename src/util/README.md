...
variable = value;
{
  GSPC_UTIL_IN_THIS_SCOPE (variable) = other_value;
...
retrieved using the helper macro `GSPC_UTIL_SCOPED_DLHANDLE_SYMBOL`
...
* `GSPC_UTIL_REQUIRED_VA_ARG`: get nth argument
* `GSPC_UTIL_OPTIONAL_VA_ARG`: get nth argument or fallback-th if not
  enough arguments given

```c++
#define OPERATOR_SIGNATURE(/*lhs, rhs = lhs*/...)                  \
  bool operator== ( GSPC_UTIL_REQUIRED_VA_ARG (1, __VA_ARGS__)     \
                  , GSPC_UTIL_OPTIONAL_VA_ARG (2, 1, __VA_ARGS__)  \
                  )
...
* `GSPC_UTIL_MAKE_COMBINED_STD_HASH` defines `std::hash` for the given
