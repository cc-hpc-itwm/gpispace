...
#define REQUIRE_ON_STRAND()                                             \
      ...
      , _io_thread ([this] { io_service_->run(); })
    {
      try
      {
        ...
        my_addr_ = p2p::address_t
          (host_t {gspc::util::hostname()}, port_t {local_endpoint().port()});
        ...
        ( [this, addr, host, port, connect_done]
          {
            try
            {
              ...
              connection_data_t& cd (connections_.at (addr));
                ...
                cd.connection = std::make_shared<connection_t>
                ( *io_service_
                , ctx_.get()
                , strand_
              ...
              cd.connection->local_address (my_addr_.value());
    ...
    {
      gspc_assert (completion_handler);
      ...
      connection_data_t & cd = connections_.at (addr);
      cd.o_queue.push_back ({{data, my_addr_.value(), addr}, completion_handler});
      ...
      (lock_type const& lock, connection_data_t& cd)
    {
      ...
      cd.o_queue.push_front
        ( { {message_t::Hello{}, my_addr_.value(), cd.connection->remote_address()}
      ...
      (connection_t::ptr_t connection, ::boost::system::error_code const& ec)
    {
      ...
      gspc_assert (connection == listen_);
    ...
    void peer_t::accept_new ()
    {
      ...
      listen_->local_address (my_addr_.value());
      ...
      (connection_t::ptr_t connection, std::unique_ptr<message_t> m)
    {
      ...
      lock_type lock (mutex_);
      {
        ...
        else
        {
          ...
          lock.unlock ();
          to_recv ( { ::boost::system::error_code{}
