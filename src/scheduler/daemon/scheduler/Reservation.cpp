...
#include <utility>
    ...
    namespace gspc::scheduler::daemon::scheduler
    {
        ...
        if (!_preferences.empty())
        {
          if (!implementation)
          {
            throw std::logic_error
              ("The implementation cannot be std::nullopt "
      ...
      gspc::we::type::Preferences Reservation::preferences() const
      {
        return _preferences;
      }
      ...
      {
        return (_results.individual_results.size() == _workers.size())
          ? std::make_optional (_results)
          : std::nullopt
          ;
      }
      ...
      bool Reservation::is_canceled() const
      {
        return std::any_of
          ...
          , [] (std::pair<gspc::scheduler::worker_id_t, terminal_state> const& result)
            {
