...
#include <vector>
  ...
  namespace gspc::scheduler::daemon
  {
    ...
    void CoallocationScheduler::assign_jobs_to_workers()
    {
      ...
      std::list<gspc::scheduler::job_id_t> nonmatching_jobs_queue;

      while (!jobs_to_schedule.empty())
      {
        gspc::scheduler::job_id_t const jobId (jobs_to_schedule.front());
          ...
          if (_strategy._worker_manager.all_free (workers))
          {
            ...
            serve_job
              ...
              , [this] (worker_id_t const& worker)
                {
                  return _strategy._worker_manager.address_by_worker (worker).value()->second;
                }
              );
    ...
    void CoallocationScheduler::release_reservation (gspc::scheduler::job_id_t const& job_id)
    {
      ...
      (job_id_t const& job, status::code const status)
    {
      ...
      if ( !_strategy.allocation_table_.at (job)->is_canceled()
        || (status == gspc::scheduler::status::CANCELING)
         )
      {
        return false;
      }
      ...
      for (auto const& worker_class : _strategy._worker_manager.classes_and_workers())
      {
        ...
        for (auto& worker_id : worker_class.second)
        {
          ...
          bpq.emplace ( total_cost
                      , -1.0 * matching_degree_and_implementation.first.value()
                      , shared_memory_size
                      , last_time_idle
                      , worker_id
    ...
    void CoallocationScheduler::submit_job (gspc::scheduler::job_id_t const& job)
    {
      _strategy.submit_job (job);
    }

    void CoallocationScheduler::acknowledge_job_sent_to_worker
      (job_id_t const& job, gspc::com::p2p::address_t const& address)
    {
      _strategy.acknowledge_job_sent_to_worker (job, address);
    }

    std::optional<job_result_type> CoallocationScheduler::
