...
namespace
{
  template<typename Exception, typename T, typename... Args>
    T get_or_throw (std::optional<T> const& optional, Args&&... args)
  {
    if (!optional)
    {
  ...
  namespace gspc::scheduler::daemon
  {
    ...
    {
      for ( gspc::we::type::Requirement const& req
      ...
      auto const preference
        ( std::find_if ( preferences.cbegin()
                       , preferences.cend()
                       , [&] (gspc::we::type::Preferences::value_type const& pref)
                         {
      ...
      std::optional<double> matching_req_and_pref_deg
    ...
    std::optional<job_result_type> CostAwareWithWorkStealingStrategy::
      store_individual_result_and_get_final_if_group_finished
        ( gspc::com::p2p::address_t const& worker_addr
    ...
    void CostAwareWithWorkStealingStrategy::notify_submitted_or_acknowledged_workers
      ( job_id_t const& job_id
      , std::function<void ( gspc::com::p2p::address_t const&)> notify_workers
      )
    {
      ...
      {
        notify_workers (_worker_manager.address_by_worker (w).value()->second);
      }
    }
    ...
    void CostAwareWithWorkStealingStrategy::reschedule_worker_jobs_and_maybe_remove_worker
      ...
      , std::function<Job* (gspc::scheduler::job_id_t const&)> get_job
      ...
      )
    {
      ...
      if (as_worker)
      {
        auto const worker_id (as_worker.value()->second);

        std::unordered_set<gspc::scheduler::job_id_t> jobs_to_reschedule
          (_worker_manager.pending_jobs (worker_id));

        std::unordered_set<gspc::scheduler::job_id_t> jobs_to_cancel
    ...
    void CostAwareWithWorkStealingStrategy::reschedule_worker_jobs_and_maybe_remove_worker
      ( gspc::com::p2p::address_t const& source
      , std::function<Job* (gspc::scheduler::job_id_t const&)> get_job
      , std::function<void (gspc::com::p2p::address_t const& addr, job_id_t const&)> cancel_worker_job
      ...
      )
    {
      ...
      if (as_worker)
      {
        auto const worker (as_worker.value()->second);
    ...
    std::unordered_set<gspc::scheduler::job_id_t> CostAwareWithWorkStealingStrategy::delete_or_cancel_worker_jobs
      ( worker_id_t const& worker_id
      , std::function<Job* (gspc::scheduler::job_id_t const&)> get_job
      , std::function<void (gspc::com::p2p::address_t const&, job_id_t const&)> cancel_worker_job
      , std::lock_guard<std::mutex> const&
      , std::lock_guard<std::mutex> const&
      )
    {
        ...
        if ( !reservation->apply_to_workers_without_result
               ( [this, &jobId, &cancel_already_requested, &cancel_worker_job]
                 (worker_id_t const& wid)
                 {
                   ...
                   {
                     cancel_worker_job
                       (_worker_manager.address_by_worker (wid).value()->second, jobId);
                   }
                 }
               )
           )
    ...
    bool CostAwareWithWorkStealingStrategy::cancel_job
      ( job_id_t const& job_id
      , std::function<void (gspc::com::p2p::address_t const&)> send_cancel
      )
    {
      std::lock_guard<std::mutex> const lock_alloc_table (mtx_alloc_table_);
      std::lock_guard<std::mutex> const lock_worker_man (_worker_manager._mutex);
      ...
      {
        send_cancel
          (_worker_manager.address_by_worker (assigned_worker).value()->second);
        return true;
      }
      else
      {
      ...
      )
    {
      ...
      {
        send_cancel (_worker_manager.address_by_worker (w).value()->second);
      }
