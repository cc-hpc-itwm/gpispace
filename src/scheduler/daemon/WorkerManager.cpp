...
#include <vector>
  ...
  namespace gspc::scheduler::daemon
  {
    ...
    void WorkerManager::delete_worker (worker_id_t const& workerId)
    {
      ...
      gspc_assert (worker != worker_map_.end(), "Worker not found when deletion was requested!");
      ...
      )
    {
      ...
      gspc_assert (worker != worker_map_.end());
    ...
    std::optional<WorkerManager::worker_connections_t::right_iterator>
      WorkerManager::worker_by_address (gspc::com::p2p::address_t const& address)
    {
      WorkerManager::worker_connections_t::right_iterator it
        (worker_connections_.right.find (address));
      return (it != worker_connections_.right.end())
        ? std::make_optional (it)
        : std::nullopt
        ;
    }

    std::optional<WorkerManager::worker_connections_t::left_iterator>
      WorkerManager::address_by_worker (std::string const& worker)
    {
      ...
      return (it != worker_connections_.left.end())
        ? std::make_optional (it)
        : std::nullopt
        ;
    }
    ...
    {
      gspc_assert ( _num_pending_jobs >= k
    ...
    {
      gspc_assert ( _num_running_jobs >= k
      ...
      )
    {
      for (auto const& weqc : worker_equiv_classes_)
      {
        ...
        for (worker_id_t const& w : _classes_and_worker_ids.at (weqc.first))
        {
          ...
          gspc_assert (it != worker_map_.end());
        ...
        while (!(thieves.empty() || to_steal_from.empty()))
        {
          ...
          gspc_assert (it_job != richest_worker.pending_.end());

          auto const preferences {reservation (*it_job)->preferences()};
      ...
      (worker_id_t const& worker_id) const
    {
      ...
      std::unordered_set<gspc::scheduler::job_id_t> sub_or_ack_jobs;
    ...
    std::optional<job_id_t> WorkerManager::get_next_worker_pending_job_to_submit
      (worker_id_t const& worker_id)
    {
