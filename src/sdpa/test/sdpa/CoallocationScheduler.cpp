...
struct fixture_scheduler_and_requirements
{
  ...
  unsigned long count_assigned_jobs ( sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY assignment
                                    , const sdpa::worker_id_t& worker_id
                                    )
  {
    auto value = [](const sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY::value_type& p)
                   {
                     return p.second;
                   };
...
BOOST_FIXTURE_TEST_CASE (load_balancing, fixture_scheduler_and_requirements)
{
  ...
  const sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY
...
struct fixture_add_new_workers
{
  ...
  sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY get_current_assignment()
  {
  ...
  std::set<sdpa::job_id_t> get_jobs_assigned_to_worker
    ...
    , sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY const& assignment
    )
  {
    BOOST_REQUIRE (!assignment.empty());
  ...
  unsigned int n_jobs_assigned_to_worker
    ( sdpa::worker_id_t const& worker
    , sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY assignment
    )
  {
  ...
  void check_work_stealing
    ...
    , sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY old_assignment
  ...
  (request_arbitrary_number_of_workers, fixture_add_new_workers)
{
  ...
  while (n_assigned_workers < n_workers)
  {
    ...
    sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY const
      assignment (get_current_assignment());
  ...
  )
{
  ...
  sdpa::daemon::CoallocationScheduler::assignment_t_TESTING_ONLY
    assignment (get_current_assignment());
