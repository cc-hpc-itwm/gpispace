// Copyright (C) 2011,2014-2016,2019-2026 Fraunhofer ITWM
...
#include <unistd.h>
    ...
    namespace gpi::pc::container
    {
      ...
      void manager_t::safe_unlink (std::string const& path)
      {
        ...
        try
        {
          gspc::util::syscall::stat (path.c_str(), &st);
        }
        ...
        gspc::util::syscall::unlink (path.c_str());
      }
      ...
      void manager_t::listener_thread_main()
      {
        ...
        for (;;)
        {
          ...
          try
          {
            cfd = gspc::util::syscall::accept ( m_socket
      ...
      namespace
      {
        struct handle_message_t
        {
          ...
          gpi::pc::proto::message_t operator()
        ...
        {
          return io_exact
            (fd, buffer, size, &gspc::util::syscall::read, "read", "read");
        }
        ...
        {
          return io_exact
            (fd, buffer, size, &gspc::util::syscall::write, "write", "written");
        }
        ...
        (gpi::pc::type::process_id_t process_id, int socket)
      {
        ...
        try
        {
          gspc::util::syscall::shutdown (socket, SHUT_RDWR);
        }
        ...
        gspc::util::syscall::close (socket);
                    ...
                    )
      {
        ...
        try
        {
          m_socket = gspc::util::syscall::socket (AF_UNIX, SOCK_STREAM, 0);
        }
        catch (...)
        {
          std::throw_with_nested
        ...
        struct close_socket_on_error
        {
          ...
          ~close_socket_on_error()
          {
            if (!_committed)
            {
              gspc::util::syscall::close (_socket);
            }
          }
        ...
        {
          const int on (1);
          gspc::util::syscall::setsockopt (m_socket, SOL_SOCKET, SO_PASSCRED, &on, sizeof (on));
        }
        ...
        {
          delete_socket_file_on_error (std::filesystem::path path)
            : _path {std::move (path)}
          {}
          ~delete_socket_file_on_error()
          {
            if (!_committed)
            {
              gspc::util::syscall::unlink (_path.string().c_str());
            }
          }
        ...
        gspc::util::syscall::chmod (m_path.c_str(), 0700);

        const std::size_t backlog_size (16);
        gspc::util::syscall::listen (m_socket, backlog_size);
