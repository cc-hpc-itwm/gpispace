// Copyright (C) 2011,2014-2015,2020-2023,2025-2026 Fraunhofer ITWM
  ...
  namespace gspc::iml::detail
  {
    namespace
    {
      ...
      void OpenedSharedMemory::create (mode_t mode)
      {
        ...
        try
        {
          fd = gspc::util::syscall::shm_open (_name.c_str(), O_RDWR | O_CREAT | O_EXCL, mode);
        }
        ...
        try
        {
          gspc::util::syscall::ftruncate (fd, _size);
        }
        catch (::boost::system::system_error const&)
        {
          std::throw_with_nested
        ...
        try
        {
          m_ptr = gspc::util::syscall::mmap ( nullptr
                                           , _size
      ...
      void OpenedSharedMemory::open ()
      {
        ...
        try
        {
          fd = gspc::util::syscall::shm_open (_name.c_str(), O_RDWR, 0);
        }
        catch (::boost::system::system_error const&)
        {
          std::throw_with_nested
      ...
      void OpenedSharedMemory::close ()
      {
        if (m_ptr)
        {
          gspc::util::syscall::munmap (m_ptr, _size);
          m_ptr = nullptr;
        }
      }
      ...
      {
        gspc::util::syscall::shm_unlink(_name.c_str());
      }
