// Copyright (C) 2014-2017,2020-2023,2025-2026 Fraunhofer ITWM
// SPDX-License-Identifier: GPL-3.0-or-later

#include <gspc/iml/vmem/gaspi/gpi/gaspi.hpp>

#include <gspc/iml/vmem/gaspi/exception.hpp>
...
#define FAIL_ON_NON_ZERO(F, Args...)             \
    ...
    namespace
    {
      ...
      {
        auto const phys_pages (gspc::util::syscall::sysconf (_SC_PHYS_PAGES));
        auto const pagesize (gspc::util::syscall::sysconf (_SC_PAGESIZE));
        return phys_pages * pagesize;
      }
    }

    gaspi_t::gaspi_t ( gspc::iml::vmem::gaspi_context& gaspi_context
      ...
      std::size_t const chunks
        ( std::accumulate
            ...
            , [&chunk_size] (std::size_t accum, transfer_part const& part)
              {
                return accum + gspc::util::divru (part.size, chunk_size);
              }
            )
        );
