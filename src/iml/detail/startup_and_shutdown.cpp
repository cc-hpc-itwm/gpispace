...
#include <gspc/iml/detail/startup_and_shutdown.hpp>
...
namespace gspc::iml
{
      ...
      )
    {
      ...
      gspc::iml::detail::Installation const installation;

      std::list<std::pair<gspc::rif::client, gspc::rif::entry_point>>
        rif_connections;
      ...
      try
      {
        std::unordered_map<gspc::rif::entry_point, std::future<pid_t>>
        ...
        if (!fails.empty())
        {
          gspc::util::throw_collected_exceptions
            ( fails
            , [] (std::pair<gspc::rif::entry_point, std::exception_ptr> const& fail)
              {
                return fmt::format
                  ( "vmem startup failed {}: {}"
                  , fail.first
                  , gspc::util::exception_printer (fail.second)
                  );
              }
            );
        }
