...
extern "C"
{
  ...
  {
    return { we::type::bytearray ( asian::roll (to<asian::param_t> (user_data)
                                               , number_of_rolls
                                               , seed
                                               )
                                 )
    ...
    )
  {
    return we::type::bytearray
      ( asian::reduce ( to<asian::reduced_type> (state)
                      , to<asian::roll_result_type> (partial_result)
                      )
      );
  }
  ...
  {
    return we::type::bytearray
      ( asian::post_process ( number_of_rolls
                            , to<asian::reduced_type> (reduced)
                            , to<asian::param_t> (user_data)
                            )
      );
  }
}
