...
void test_Defrag(Module::data_t &params)
{
  ...
  for(int i=0; i < 8; i++)
    {    
      allocH[i] = fvmGlobalAlloc((128 * 1024 * 1024));
      assert(allocH[i] > 0);
    }
  ...
  src = fvmGlobalAlloc((256 * 1024 * 1024));
  assert(src > 0);
...
void test_GlobalMemory(Module::data_t &params)
{
  ...
  fvmAllocHandle_t src1 = fvmGlobalAlloc((256*1024*1024));
  assert(src1 > 0);

  fvmAllocHandle_t src2 = fvmGlobalAlloc((256*1024*1024));
  assert(src2 > 0);

  fvmAllocHandle_t src3 = fvmGlobalAlloc((256*1024*1024));
  assert(src3 > 0);

  fvmAllocHandle_t src4 = fvmGlobalAlloc((256*1024*1024));
  assert(src4 == 0);
