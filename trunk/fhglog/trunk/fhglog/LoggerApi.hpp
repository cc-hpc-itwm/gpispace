...
#include <fhglog/Logger.hpp>
  ...
  class LoggerApi {
    public:
      ...
      inline Appender::ptr_t addAppender(Appender::ptr_t appender) { return impl_->addAppender(appender); }
      inline Appender::ptr_t getAppender(const std::string &appender_name) const { return impl_->getAppender(appender_name); }
  ...
  {
    return LoggerApi(Logger::get(name).get());
  }
#else
