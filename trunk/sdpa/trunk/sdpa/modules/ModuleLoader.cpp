...
void ModuleLoader::unload(const std::string &module) {
...
void ModuleLoader::unload(module_table_t::iterator mod) {
...
const Module& ModuleLoader::get(const std::string &module) const throw(ModuleNotLoaded) {
...
Module& ModuleLoader::load(const std::string &module, const std::string &file) throw (ModuleLoadFailed) {
  char *error = 0;
  ...
  Module::Ptr mod(new Module(module, handle));
