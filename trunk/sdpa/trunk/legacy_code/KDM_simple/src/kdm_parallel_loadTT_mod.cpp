...
static void initialize (void *, const we::loader::input_t & input, we::loader::output_t & output)
{
  ...
  long wait (0);
  long parallel_loadTT (0);
  const value::type & config (kdm_initialize (filename, wait, parallel_loadTT));
...
static void loadTT (void *, const we::loader::input_t & input, we::loader::output_t & output)
{
  ...
  we::loader::put_output (output, "TT", literal::type(TT));
}
...
WE_MOD_INITIALIZE_START (kdm_parallel_loadTT);
{
  LOG(INFO, "WE_MOD_INITIALIZE_START (kdm_parallel_loadTT)");
...
WE_MOD_INITIALIZE_END (kdm_parallel_loadTT);

WE_MOD_FINALIZE_START (kdm_parallel_loadTT);
{
  LOG(INFO, "WE_MOD_FINALIZE_START (kdm_parallel_loadTT)");
}
WE_MOD_FINALIZE_END (kdm_parallel_loadTT);
