/***************************************************************************
                          ttitracingoperator.cpp  -  description
                             -------------------
...
ttiTracingOperator<VelModelType>::ttiTracingOperator():VelModel(NULL),
						 VhModel(NULL),
						 ExModel(NULL),
						 TTIAlphaModel(NULL),
						 TTIBetaModel(NULL)
...
template<class VelModelType>
ttiTracingOperator<VelModelType>::~ttiTracingOperator(){
}

/** operator () (const ttiTracingOperator& ) */
template<class VelModelType>
void ttiTracingOperator<VelModelType>::operator () (ray3D& ray, const int ith)
{
  ray.Store();
...
/** operator () (const ttiTracingOperator& ) */
template<class VelModelType>
void ttiTracingOperator<VelModelType>::operator () (ray3D* rays[4], const int ith)
{
  for (int n = 0; n < 4; n++)
    rays[n]->Store();
...
/** No descriptions */
template<class VelModelType>
void ttiTracingOperator<VelModelType>::Init(const float& _dt){
  if (_dt <= 0)
    {
      std::cerr << "FATAL ERROR in ttiTracingOperator<VelModelType>::Init\n";
...
/** No descriptions */
template<class VelModelType>
inline void ttiTracingOperator<VelModelType>::Eval_at(const point3D<float>& x, const point3D<float>& p, const double Q[3][3], const double P[3][3], 
...
template<class VelModelType>
inline void ttiTracingOperator<VelModelType>::TraceTstep_TTIRungeKutta_dynamic(const double _dt, ray3D & ray, const int ith)
{
    ...
    point3D<float> k1_x_rotated, k2_x_rotated, k3_x_rotated, k4_x_rotated;
    point3D<float> k1_p_rotated, k2_p_rotated, k3_p_rotated, k4_p_rotated;

    double k1_Q[3][3];
    double k1_P[3][3];
    double k1_Q_rotated[3][3];
    double k1_P_rotated[3][3];

    double k2_Q[3][3];
    double k2_P[3][3];
    double k2_Q_rotated[3][3];
    double k2_P_rotated[3][3];

    double k3_Q[3][3];
    double k3_P[3][3];
    double k3_Q_rotated[3][3];
    double k3_P_rotated[3][3];

    double k4_Q[3][3];
    double k4_P[3][3];
    double k4_Q_rotated[3][3];
    double k4_P_rotated[3][3];

    double Q[3][3];
    double P[3][3];
    double P_rotated[3][3];

    point3D<float> x_tmp = ray.x;
    point3D<float> p_tmp = ray.p;

    {
	const point3D<float> x_RK(ray.x);
	const point3D<float> p_RK(ray.p);
	for (int i = 0; i < 3; i++)
	    for (int k = 0; k < 3; k++)
	    {
		Q[i][k] = ray.Q[i][k];
	...
	const float alpha = deg2rad(TTIAlphaModel->GetProperty(ray.x, ith));
	const float beta = deg2rad(TTIBetaModel->GetProperty(ray.x, ith));
	const float calpha = cosf(alpha);
	const float salpha = sinf(alpha);
	const float cbeta = cosf(beta);
	const float sbeta = sinf(beta);
	...
	const point3D<float> p_RK_rotated( R00*p_RK[0] + R01*p_RK[1] + R02*p_RK[2],
					   R10*p_RK[0] + R11*p_RK[1] + R12*p_RK[2],
					   R20*p_RK[0] + R21*p_RK[1] + R22*p_RK[2]);

	for (int i = 0; i < 3; i++)
	{
	    P_rotated[i][0] = R00*P[i][0] + R01*P[i][1] + R02*P[i][2];
	    P_rotated[i][1] = R10*P[i][0] + R11*P[i][1] + R12*P[i][2];
	    P_rotated[i][2] = R20*P[i][0] + R21*P[i][1] + R22*P[i][2];
	}

	Eval_at(x_RK, p_RK_rotated, Q, P_rotated, k1_x_rotated, k1_p_rotated, k1_Q_rotated, k1_P_rotated, ith);

	k1_x = point3D<float>( R00*k1_x_rotated[0] + R10*k1_x_rotated[1] + R20*k1_x_rotated[2],
			       R01*k1_x_rotated[0] + R11*k1_x_rotated[1] + R21*k1_x_rotated[2],
			       R02*k1_x_rotated[0] + R12*k1_x_rotated[1] + R22*k1_x_rotated[2]);
	k1_p = point3D<float>( R00*k1_p_rotated[0] + R10*k1_p_rotated[1] + R20*k1_p_rotated[2],
			       R01*k1_p_rotated[0] + R11*k1_p_rotated[1] + R21*k1_p_rotated[2],
			       R02*k1_p_rotated[0] + R12*k1_p_rotated[1] + R22*k1_p_rotated[2]);
	for (int i = 0; i < 3; i++)
	{
	    k1_Q[i][0] = R00*k1_Q_rotated[i][0] + R10*k1_Q_rotated[i][1] + R20*k1_Q_rotated[i][2];
	    k1_Q[i][1] = R01*k1_Q_rotated[i][0] + R11*k1_Q_rotated[i][1] + R21*k1_Q_rotated[i][2];
	    k1_Q[i][2] = R02*k1_Q_rotated[i][0] + R12*k1_Q_rotated[i][1] + R22*k1_Q_rotated[i][2];

	    k1_P[i][0] = R00*k1_P_rotated[i][0] + R10*k1_P_rotated[i][1] + R20*k1_P_rotated[i][2];
	    k1_P[i][1] = R01*k1_P_rotated[i][0] + R11*k1_P_rotated[i][1] + R21*k1_P_rotated[i][2];
	    k1_P[i][2] = R02*k1_P_rotated[i][0] + R12*k1_P_rotated[i][1] + R22*k1_P_rotated[i][2];
	}
    }

    {
	const point3D<float> x_RK(ray.x + k1_x * 0.5 * (float)_dt);
	const point3D<float> p_RK(ray.p + k1_p * 0.5 * (float)_dt);
	for (int i = 0; i < 3; i++)
	    for (int k = 0; k < 3; k++)
	    {
	...
	Eval_at(x_RK, p_RK_rotated, Q, P_rotated, k2_x_rotated, k2_p_rotated, k2_Q_rotated, k2_P_rotated, ith);

	k2_x = point3D<float>( R00*k2_x_rotated[0] + R10*k2_x_rotated[1] + R20*k2_x_rotated[2],
			       R01*k2_x_rotated[0] + R11*k2_x_rotated[1] + R21*k2_x_rotated[2],
			       R02*k2_x_rotated[0] + R12*k2_x_rotated[1] + R22*k2_x_rotated[2]);
	k2_p = point3D<float>( R00*k2_p_rotated[0] + R10*k2_p_rotated[1] + R20*k2_p_rotated[2],
			       R01*k2_p_rotated[0] + R11*k2_p_rotated[1] + R21*k2_p_rotated[2],
			       R02*k2_p_rotated[0] + R12*k2_p_rotated[1] + R22*k2_p_rotated[2]);
	for (int i = 0; i < 3; i++)
	{
	    k2_Q[i][0] = R00*k2_Q_rotated[i][0] + R10*k2_Q_rotated[i][1] + R20*k2_Q_rotated[i][2];
	    k2_Q[i][1] = R01*k2_Q_rotated[i][0] + R11*k2_Q_rotated[i][1] + R21*k2_Q_rotated[i][2];
	    k2_Q[i][2] = R02*k2_Q_rotated[i][0] + R12*k2_Q_rotated[i][1] + R22*k2_Q_rotated[i][2];

	    k2_P[i][0] = R00*k2_P_rotated[i][0] + R10*k2_P_rotated[i][1] + R20*k2_P_rotated[i][2];
	    k2_P[i][1] = R01*k2_P_rotated[i][0] + R11*k2_P_rotated[i][1] + R21*k2_P_rotated[i][2];
	    k2_P[i][2] = R02*k2_P_rotated[i][0] + R12*k2_P_rotated[i][1] + R22*k2_P_rotated[i][2];
	}
    }

    {
	const point3D<float> x_RK(ray.x + k2_x * 0.5 * (float)_dt);
	const point3D<float> p_RK(ray.p + k2_p * 0.5 * (float)_dt);
	for (int i = 0; i < 3; i++)
	    for (int k = 0; k < 3; k++)
	    {
	...
	Eval_at(x_RK, p_RK_rotated, Q, P_rotated, k3_x_rotated, k3_p_rotated, k3_Q_rotated, k3_P_rotated, ith);

	k3_x = point3D<float>( R00*k3_x_rotated[0] + R10*k3_x_rotated[1] + R20*k3_x_rotated[2],
			       R01*k3_x_rotated[0] + R11*k3_x_rotated[1] + R21*k3_x_rotated[2],
			       R02*k3_x_rotated[0] + R12*k3_x_rotated[1] + R22*k3_x_rotated[2]);
	k3_p = point3D<float>( R00*k3_p_rotated[0] + R10*k3_p_rotated[1] + R20*k3_p_rotated[2],
			       R01*k3_p_rotated[0] + R11*k3_p_rotated[1] + R21*k3_p_rotated[2],
			       R02*k3_p_rotated[0] + R12*k3_p_rotated[1] + R22*k3_p_rotated[2]);
	for (int i = 0; i < 3; i++)
	{
	    k3_Q[i][0] = R00*k3_Q_rotated[i][0] + R10*k3_Q_rotated[i][1] + R20*k3_Q_rotated[i][2];
	    k3_Q[i][1] = R01*k3_Q_rotated[i][0] + R11*k3_Q_rotated[i][1] + R21*k3_Q_rotated[i][2];
	    k3_Q[i][2] = R02*k3_Q_rotated[i][0] + R12*k3_Q_rotated[i][1] + R22*k3_Q_rotated[i][2];

	    k3_P[i][0] = R00*k3_P_rotated[i][0] + R10*k3_P_rotated[i][1] + R20*k3_P_rotated[i][2];
	    k3_P[i][1] = R01*k3_P_rotated[i][0] + R11*k3_P_rotated[i][1] + R21*k3_P_rotated[i][2];
	    k3_P[i][2] = R02*k3_P_rotated[i][0] + R12*k3_P_rotated[i][1] + R22*k3_P_rotated[i][2];
	}
    }

    {
	const point3D<float> x_RK(ray.x + k3_x * (float)_dt);
	const point3D<float> p_RK(ray.p +  k3_p * (float)_dt);
	for (int i = 0; i < 3; i++)
	    for (int k = 0; k < 3; k++)
	    {
	...
	Eval_at(x_RK, p_RK_rotated, Q, P_rotated, k4_x_rotated, k4_p_rotated, k4_Q, k4_P_rotated, ith);

	k4_x = point3D<float>( R00*k4_x_rotated[0] + R10*k4_x_rotated[1] + R20*k4_x_rotated[2],
			       R01*k4_x_rotated[0] + R11*k4_x_rotated[1] + R21*k4_x_rotated[2],
			       R02*k4_x_rotated[0] + R12*k4_x_rotated[1] + R22*k4_x_rotated[2]);
 	k4_p = point3D<float>( R00*k4_p_rotated[0] + R10*k4_p_rotated[1] + R20*k4_p_rotated[2],
 			       R01*k4_p_rotated[0] + R11*k4_p_rotated[1] + R21*k4_p_rotated[2],
 			       R02*k4_p_rotated[0] + R12*k4_p_rotated[1] + R22*k4_p_rotated[2]);
	for (int i = 0; i < 3; i++)
	{
	    k4_Q[i][0] = R00*k4_Q_rotated[i][0] + R10*k4_Q_rotated[i][1] + R20*k4_Q_rotated[i][2];
	    k4_Q[i][1] = R01*k4_Q_rotated[i][0] + R11*k4_Q_rotated[i][1] + R21*k4_Q_rotated[i][2];
	    k4_Q[i][2] = R02*k4_Q_rotated[i][0] + R12*k4_Q_rotated[i][1] + R22*k4_Q_rotated[i][2];

	    k4_P[i][0] = R00*k4_P_rotated[i][0] + R10*k4_P_rotated[i][1] + R20*k4_P_rotated[i][2];
	    k4_P[i][1] = R01*k4_P_rotated[i][0] + R11*k4_P_rotated[i][1] + R21*k4_P_rotated[i][2];
	    k4_P[i][2] = R02*k4_P_rotated[i][0] + R12*k4_P_rotated[i][1] + R22*k4_P_rotated[i][2];
	}
    }
...
template<class VelModelType>
double ttiTracingOperator<VelModelType>::TraceToSurface(ray3D& ray, const float finaldepth, const int t, const int ith)
{
...
template<class VelModelType>
double ttiTracingOperator<VelModelType>::TraceToPoint(ray3D& ray, const point3D<float>& DestPoint, const int ith)
{
...
template<class VelModelType>
float ttiTracingOperator<VelModelType>::GetTT(const float dt, const int tstepindex)
{
...
template<class VelModelType>
void ttiTracingOperator<VelModelType>::BindModels(const VelModelRep_t& VModels)
  {
    VelModel = VModels.V;
    VhModel = VModels.Vh;
    ExModel = VModels.Ex;
    TTIAlphaModel = VModels.Alpha;
    TTIBetaModel = VModels.Beta;
  };

template<class VelModelType>
void ttiTracingOperator<VelModelType>::GetVelocityAt(tti_velocity& tti, const point3D<float>& Point, const int ith)
{
    tti.v = VelModel->GetProperty(Point, ith);
    tti.A11 = VhModel->GetProperty(Point, ith)*this->VhModel->GetProperty(Point, ith);
    tti.Ex = ExModel->GetProperty(Point, ith);
    tti.alpha_rad = deg2rad(TTIAlphaModel->GetProperty(Point, ith));
    tti.beta_rad = deg2rad(TTIBetaModel->GetProperty(Point, ith));
}
