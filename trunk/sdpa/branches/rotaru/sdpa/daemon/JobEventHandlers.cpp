...
void GenericDaemon::handleCancelJobEvent(const CancelJobEvent* pEvt )
{
	...
	if(pEvt->from() == master() ) //the master sent a Cancel message -> inform the workfloe engine
	{
		try {
			...
			// else, forward the cancel to the GWES
			if( pJob->getStatus().find("Pending") == std::string::npos ) // not in Pending
			{
				workflow_id_t workflowId = pEvt->job_id();
	...
	else // the event was  generated by GWES
	{
		try {

            sdpa::worker_id_t worker_id = pJob->get("worker");
