...
sdpa::daemon::GenericDaemon::job_map_t GenericDaemon :: GetJobList() { return job_map_; }
...
void GenericDaemon :: action_submit_job(const sdpa::events::SubmitJobEvent& e) {
	...
	*/
	...
	sdpa::uuidgen gen;
	gen(id);
	sdpa::job_id_t strJobID = id.str();

	Job::ptr_t pJob( new sdpa::fsm::smc::JobFSM( strJobID, e.description() ));
	job_map_.insert( make_pair( strJobID, pJob) );
	std::cout<<"Inserted "<<"Job "<<strJobID<<" into the job map"<<std::endl;
}
