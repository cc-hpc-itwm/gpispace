...
{
	return transit<Configurring>(&OrchFSM::action_configure, e); // successfully configured and all services started-up
}
...
{
	return transit<Up>(&OrchFSM::action_config_ok, e);
}

sc::result Configurring::react( const ConfigNokEvent& e)
{
	return transit<Down>(&OrchFSM::action_config_nok, e);
}
...
{
   	return transit<Up>(&OrchFSM::action_submit_job, e);
}

sc::result Up::react(const SubmitJobAckEvent& e )
{
   	return transit<Up>(&OrchFSM::action_submit_job_ack, e);
}
