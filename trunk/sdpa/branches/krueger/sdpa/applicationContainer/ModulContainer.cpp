...
ModulContainer::ModulContainer(std::string sFilename) :
...
void *ModulContainer::_dynl_open()  {
  return(dlopen(m_sFilename.c_str(), RTLD_NOW|RTLD_GLOBAL));
}

void *ModulContainer::_dynl_sym(const char *sSymbol) {
  return(dlsym(m_pHandle, sSymbol));
}

int ModulContainer::_dynl_close() 
{
