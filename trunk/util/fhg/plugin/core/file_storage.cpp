...
namespace fhg
{
  namespace core
  {
    ...
    FileStorage::FileStorage(path_t const & path, bool do_restore)
      : m_path (path)
    {
      if (do_restore)
        restore();
    ...
    fhg::plugin::Storage *FileStorage::get_storage(std::string const&) const
    {
      return 0;
    }
    ...
    int FileStorage::save (std::string const &key, std::string const &value)
    {
      ...
      lock_type lock(m_transaction_mutex);
      m_transaction[key] = val;
      return 0;
    }
    ...
    bool FileStorage::exists (std::string const &key) const
    {
      return false;
    }
