...
int main(int argc, char **argv)
{
  ...
  /* Get parameters */
  if (!getparint("nx", &Vnx)) err("must specify nx=");
  if (!getparint("ny", &Vny)) err("must specify ny=");
  if (!getparint("nz", &Vnz)) err("must specify nz=");
  if (!getparfloat("dx", &dx)) err("must specify dx=");
  if (!getparfloat("dy", &dy)) err("must specify dy=");
  if (!getparfloat("dz", &dz)) err("must specify dz=");
  if (!getparfloat("zmax", &zmax)) zmax=0.0f;

  if (!getparfloat("lx", &lx)) lx=6000;
  if (!getparfloat("ly", &ly)) ly=6000;
  if (!getparfloat("f1", &f1)) f1=4;
  if (!getparfloat("f2", &f2)) f2=7;
  if (!getparfloat("f3", &f3)) f3=20;
  if (!getparfloat("f4", &f4)) f4=26;
  if (!getparfloat("pad", &pad)) pad=0.0f;
  if (!getparfloat("latSamplesPerWave", &latSamplesPerWave)) latSamplesPerWave=3.5;
  if (!getparfloat("vertSamplesPerWave", &vertSamplesPerWave)) vertSamplesPerWave=3.5;

  if (!getparstring("medium", &medium)) medium="ISO";
  if (strcmp(medium,"ISO")&&strcmp(medium,"VTI")&&strcmp(medium,"TTI")) {
    err("wrong medium=");
  }
  if (!getparstring("propagator", &propagator)) propagator="PS";
  if (strcmp(propagator,"PS")&&strcmp(propagator,"SSF")&&strcmp(propagator,"FFD2")
      &&strcmp(propagator,"FFD3")) {
    err("wrong propagator=");
  }

  if (!getparstring("vPFile", &vPFile)) err("must specify vPFile=");
  mvP = mods_fopen(vPFile, "r");
  if (!strcmp(medium,"VTI")) {
    if (!getparstring("eFile", &eFile)) err("must specify eFile=");
    if (!getparstring("dFile", &dFile)) err("must specify dFile=");
  ...
  nsegy = gettr(&tr);
  if (!nsegy) err("cant't read first trace");
  ...
  // Run owmg_init
  data = owmg_init(medium, propagator, nx, ny, nz, dx, dy, dz, iw1, iw4, nwH, dw, latSamplesPerWave, vertSamplesPerWave);
  ...
  for (int iw=0 ; iw<nwH; iw++) {
    if ( (iw+1>=(iw1-iw1)) && (iw+1<=(iw2-iw1)) && (iw1!=iw2) ) {
      taperw[iw] = 0.53824f-0.46164f*cosf((float)PI*(iw+1)/(iw2-iw1));
    } else if ( (iw+1>=(iw3-iw1+1)) && (iw+1<=(iw4-iw1+1)) && (iw3!=iw4) ) {
      taperw[iw] = 0.53824f-0.46164f*cosf((float)PI*(iw+iw1-iw3)/(iw4-iw3)-(float)PI);
    } else {
      taperw[iw] = 1.0f;
    }
    //fprintf(stderr, "%f\n", taperw[iw]);
  ...
  while (nsegy) {		     /* While previous trace non-empty */
    ...
    if (valcmp(type1, val1, valnew1) || valcmp(type2, val2, valnew2) || !nsegy) {
...
void addRecTrace(owmgData *data)
{
  ...
  if ( (ix>=0) && (ix<data->nx) && (iy>=0) && (iy<data->ny) ) {
    ...
    float dist=powf(gx-ix*dx-Lox,2.0f)+powf(gy-iy*dy-Loy,2.0f);
...
void readModel(modsFILE *mFILE, float ***cube, owmgData *data)
{
  ...
  for ( ix=0; ix<data->nxf*data->nyf*data->nz; ix++)  cube[0][0][ix]=-2.0;

  // Loop over y
  for ( iy=iVoy+pady1; iy<=iVoy+data->nyf-1-pady2; iy++ ) {

    // Read from disk
    for ( ix=0; ix<data->nxf*Vnz; ix++)  nvec[0][ix]=-1.0;
    mods_fread(mFILE, nvec[padx1], iy*Vnx*Vnz+(iVox+padx1)*Vnz, (data->nxf-padx2-padx1)*Vnz);
