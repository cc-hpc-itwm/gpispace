...
//       else if (ready == 0)
//       {
//         LOG(WARN, "poll() timed out!");
//         close (pipe_[0][0]); pipe_[0][0] = -1;
//         close (pipe_[1][0]); pipe_[1][0] = -1;
//         close (pipe_[2][0]); pipe_[2][0] = -1;
//       }
//       else
//       {
//         DLOG(TRACE, ready << " ready file descriptor(s)");
        
//         for (int fd (0); fd < sizeof(poll_fd) && ready; ++fd)
//         {
//           sigpipe_occured = false;
          
//           struct pollfd * pfd = poll_fd+fd;
//           if (pfd->revents)
//           {
//             --ready;
            
//             DLOG(TRACE, "revents = " << pfd->revents);
//             if (pfd->revents & POLLHUP)
//             {
//               LOG(WARN, "hangup on fd " << pfd->fd);
//             }
            
//             if (pfd->revents & POLLERR)
//             {
//               LOG(WARN, "an error occured on fd " << pfd->fd);
//               close (pfd->fd);
//               if      (pfd->fd == pipe_[0][0]) pipe_[0][0] = -1;
//               else if (pfd->fd == pipe_[1][0]) pipe_[1][0] = -1;
//               else if (pfd->fd == pipe_[2][0]) pipe_[2][0] = -1;
//               else
//               {
//                 throw std::runtime_error ("STRANGE file descriptor occured!");
//               }
//               continue;
//             }
              
//             if (pfd->fd == pipe_[0][0] && pfd->revents & POLLOUT)
//             {
//               DLOG(TRACE, "writing to input stream is possible");
//               int wr = write ( pfd->fd
//                              , src
//                              , std::min( buffer_size
//                                        , (input_size - bytes_wr)
...
//               }
//             }
//             else if (pfd->fd == pipe_[1][0] && pfd->revents & POLLIN)
//             {
//               DLOG(TRACE, "reading from output stream is possible");
//               int rd = read (pfd->fd, dst, output_size - bytes_rd);
//               if (rd < 1 || sigpipe_occured)
//               {
//                 DLOG(TRACE, "closing output stream");
//                 close (pfd->fd);
//                 pipe_[1][0] = -1;
...
//               }
//             }
//             else if (pfd->fd == pipe_[2][0] && pfd->revents & POLLIN)
//             {
//               DLOG(TRACE, "reading from error stream is possible");
//               int rd = read (pfd->fd, err, error_size - bytes_er);
//               if (rd < 1 || sigpipe_occured)
//               {
//                 DLOG(TRACE, "closing error stream");
//                 close (pfd->fd);
...
//             else
//             {
//               LOG(ERROR, "STRANGE poll result!");
//               throw std::runtime_error ("STRANGE poll result!");
//             }
//           }
//            }
//          }
