...
//--------- readParamFile -------------
static int readParamFile(TReGlbStruct *pReG)
{
    const int LEN = 100;
	int i;
	FILE *fp;
	char charatemp[LEN]; 
        char prefix[LEN]; //"/gpfs/scratch/cluster/dimiter/DERE/reVM/";
        char parfile[LEN];
        char title[LEN];

        strcpy(prefix, "/scratch/dimiter/sdpa/");
        //strcpy(prefix, "/gpfs/scratch/cluster/dimiter/DERE/reVM/");
        //strcpy(prefix, "/gpfs/scratch/cluster/dimiter/DERE/reVM/3rdEx/");

        strcpy(charatemp, prefix);
        strcat(charatemp, "depth_remig3D.in");
        strcpy(parfile, charatemp);
...
char datafile[LEN];
 int sufile;
	     ...
	     //----- zero offset image ------------
	     bzero(datafile, LEN);   // dms -> my addition, ZERO the filename
             fscanf(fp, "%s %d\n", datafile, &sufile);               
	     fscanf(fp, "%\n");    

             strcpy(charatemp, prefix);
             strcat(charatemp, datafile);
             strcpy(datafile, charatemp);
...
int feet, vbackgr;
	     ...
	     //----- velocity file ------------
	     bzero(velfile, LEN);   // dms -> my addition, ZERO the filename
             fscanf(fp, "%s %d %d\n", velfile, &feet, &vbackgr);  
	     fscanf(fp, "%\n");    
 
             strcpy(charatemp, prefix);
             strcat(charatemp, velfile);
             strcpy(velfile, charatemp);
...
char outfile[LEN];
	     ...
	     fscanf(fp, "%\n");    
             ...
             strcpy(charatemp, prefix);
             strcat(charatemp, outfile);
             strcpy(outfile, charatemp);
...
//------- writeCheckParamFile -----------
static int writeCheckParamFile(TReGlbStruct *pReG)
// dms -> my addition, just to debug
{
	...
	char datafile[LEN]; 
        ...
        strcpy(outfile, "/scratch/dimiter/sdpa/Remig2000.bin");
        strcpy(title, "just a test");  


	//===== dms, write what you have read -> on each CPU ==
        sprintf(fn, "/scratch/dimiter/sdpa/checkParFile.0"); //, rank);
        //sprintf(fn, "/gpfs/scratch/cluster/dimiter/DERE/reVM/checkParFile.cpu%d", rank);
...
//----------- printParameters ----------
static int printParameters(TReGlbStruct *pReG)
{
    //!*******************************
	...
	//===== dms, write what you have read -> on each CPU ==
		...
		fprintf(fp, "\n \n\n\n printing parameters\n");
            ...
            fprintf(fp, "\n Zero offset demig file:\t %s \t (%5d, %5d, %5d)",
		                                datafile,pReG->nt,pReG->nx_out,pReG->ny_out);
            fprintf(fp, "\n Velocity file (x,y,z):\t %s \t (%5d, %5d, %5d)",
		                                velfile,pReG->nx_in,pReG->ny_in,pReG->nz);
            fprintf(fp, "\n Output file (z,x,y):\t %s \t (%5d, %5d, %5d)",
	                                        outfile, pReG->nz,pReG->nx_in,pReG->ny_in);
	    // if (propagator == PS) {
	        fprintf(fp, "\n Propagator: PS");
		//}
...
static int initReGlbVars(cfg_t *pCfg, TReGlbStruct *pReGlb)
{
       ...
       readParamFile(pReGlb);
       writeCheckParamFile(pReGlb);
 
       defineAndCalcParameters(pReGlb);
       printParameters(pReGlb);
       ...
       fvmAllocHandle_t hScra; // = pCfg[rank].hndScratch;
       hScra = fvmLocalAlloc(transfrSZbytes);
       ...
       for(iNd = 0; iNd < size; iNd++) {
           ...
           waitComm(commH);
           if(commH != COMM_HANDLE_OK) return (-1);
       } //for(iNd = 0; iNd < size; iNd++)
