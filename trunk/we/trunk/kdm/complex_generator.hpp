...
namespace kdm
{
  ...
  struct kdm
  {
    ...
    static transition_type generate (void)
    {
      ...
      transition_type gen_config
        ( "gen_config"
        , expr_type
        ( "${config.OFFSETS} := 2L;"
        "${config.BUNCHES_PER_OFFSET} := 3L;"
        "${config.STORES} := 5L;"
        "${config.SUBVOLUMES_PER_OFFSET} := 5L;"
        "${trigger} := [];"
        "${wanted} := bitset_insert ({}, 0L);"
        "${wait} := ${config.SUBVOLUMES_PER_OFFSET};"
        )
        , "true"
        , transition_type::internal
        );
      gen_config.add_ports ()
      ...
      gen_config.add_connections ()
        ...
        ;
      tid_t tid_gen_config (net.add_transition (gen_config));

      net.add_edge (e++, connection_t (PT, tid_gen_config, pid_config_file));
      net.add_edge (e++, connection_t (TP, tid_gen_config, pid_config));
      net.add_edge (e++, connection_t (TP, tid_gen_config, pid_trigger_loadTT));
      net.add_edge (e++, connection_t (TP, tid_gen_config, pid_wanted_offset));
      net.add_edge (e++, connection_t (TP, tid_gen_config, pid_volume_wait));
      ...
      transition_type store_break
        ( "gen_store_break"
        , expr_type ("${trigger} := []")
        , "${state.state} >= ${state.num}"
        , transition_type::internal
        );
      store_break.add_ports ()
        ("trigger", literal::CONTROL, we::type::PORT_OUT)
        ("state", signature::state, we::type::PORT_IN)
        ;
      store_break.add_connections ()
        ("trigger", pid_trigger_gen_offset)
        (pid_gen_store_state, "state")
        ;

      tid_t tid_gen_store_break (net.add_transition (store_break));
      ...
      transition_type gen_offset
        ( "gen_offset"
        , expr_type ("${state.num} := ${config.OFFSETS};"
                    "${state.state} := 0L"
                    )
        );
      ...
      gen_bunch.add_ports ()
        ...
        ("config", signature::config, we::type::PORT_IN_OUT)
        ;
      ...
      transition_type gen_bunch_step
        ( "gen_bunch_step"
        , expr_type ( "${bunch.id} := ${state.state.state};"
      ...
      transition_type gen_volume
        ( "gen_volume"
        , expr_type ( "${state.state.state} := 0L;"
      ...
      transition_type write
        ( "write"
        , expr_type ("${volume_written} := ${volume_to_be_written}")
        );
      write.add_ports ()
      ...
      transition_type volume_next_offset
        ( "volume_next_offset"
        , expr_type ("${wanted_offset} := bitset_insert ( ${wanted_offset} \
