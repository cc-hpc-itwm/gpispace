...
//---------- writeOutput ---------------------
static int writeOutput(cfg_t *pCfg, TReGlbStruct *pReG, float ***pCube)
{
    FILE *pfOut;
    ...
    if((pfOut=fopen(pCfg->output_file, "wb"))==NULL) {    // write at the end of the file
	  MLOG(ERROR, "could not open output file: " << pCfg->output_file);
	  return -1;
    }	
...
int reWriteOutp(cfg_t *pCfg)
{
   ...
   TReGlbStruct *pReGlb = new TReGlbStruct;
	...
	writeOutput(pCfg, pReGlb, pTrnsp);
