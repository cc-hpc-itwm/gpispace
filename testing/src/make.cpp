// Copyright (C) 2014-2015,2019-2024,2026 Fraunhofer ITWM
// SPDX-License-Identifier: GPL-3.0-or-later

#include <gspc/testing/make.hpp>

#include <gspc/testing/make_environment.hpp>
...
namespace gspc::testing
{
  ...
  make::make ( gspc::installation const& installation
    ...
    , _pnet ( static_cast<std::filesystem::path> (_build_directory)
            / (_main + ".pnet")
            )
  {
    std::filesystem::path const wrapper_directory
      (static_cast<std::filesystem::path> (_build_directory) / "gen");

    {
      ...
      gspc::util::system_with_blocked_SIGCHLD (command.str());
    }
