...
BOOST_AUTO_TEST_CASE (test_best_prefix_match)
{
  ...
  BOOST_REQUIRE_EQUAL (demux.handle_request ("/echo", rqst_frame, &user), 0);
  BOOST_REQUIRE_EQUAL (service_A.counter, 0);
  BOOST_REQUIRE_EQUAL (service_B.counter, 1);

  BOOST_REQUIRE_EQUAL (demux.handle_request ("/echo/A", rqst_frame, &user), 0);
...
BOOST_AUTO_TEST_CASE (test_best_prefix_match_no_separator)
{
  ...
  BOOST_REQUIRE_EQUAL ( demux.handle_request ("/echo", rqst_frame, &user)
                      , gspc::net::E_SERVICE_LOOKUP
                      );
  BOOST_REQUIRE_EQUAL (service_A.counter, 0);
  BOOST_REQUIRE_EQUAL (service_B.counter, 0);

  BOOST_REQUIRE_EQUAL (demux.handle_request ("/echoA", rqst_frame, &user), 0);
  BOOST_REQUIRE_EQUAL (service_A.counter, 1);
  BOOST_REQUIRE_EQUAL (service_B.counter, 0);

  BOOST_REQUIRE_EQUAL (demux.handle_request ("/echoB", rqst_frame, &user), 0);
  BOOST_REQUIRE_EQUAL (service_A.counter, 1);
  BOOST_REQUIRE_EQUAL (service_B.counter, 1);

  BOOST_REQUIRE_EQUAL ( demux.handle_request ("/echoAB", rqst_frame, &user)
                      , gspc::net::E_SERVICE_LOOKUP
                      );
  BOOST_REQUIRE_EQUAL (service_A.counter, 1);
  BOOST_REQUIRE_EQUAL (service_B.counter, 1);
...
BOOST_AUTO_TEST_CASE (test_best_prefix_match_subservice)
{
  ...
  BOOST_REQUIRE_EQUAL ( demux.handle_request ("/echoA", rqst_frame, &user)
                      , gspc::net::E_SERVICE_LOOKUP
                      );
  BOOST_REQUIRE_EQUAL (service.counter, 3);
}
...
BOOST_AUTO_TEST_CASE (test_catch_all_service)
{
  ...
  BOOST_REQUIRE_EQUAL (demux.handle_request ("/", rqst_frame, &user), 0);
