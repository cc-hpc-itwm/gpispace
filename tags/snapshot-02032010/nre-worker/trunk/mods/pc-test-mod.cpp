...
static void test_global_alloc(data_t &p) throw (std::exception)
{
  MLOG(INFO, "testing global allocation...");
  const std::size_t num_ints = p.at("size").token().data_as<std::size_t>();
  fvm::util::global_allocation test_alloc(num_ints * sizeof(int));
  ASSERT_GALLOC(test_alloc);
  ...
  // put memory to local allocation
  fvmCommHandle_t comm = fvmPutGlobalData(test_alloc
									    , 0
									    , test_alloc.size
									    , 0
										, 0
  ...
  if (commState != COMM_HANDLE_OK)
  {
	MLOG(ERROR, "fvmPutGlobalData() failed: " << commState);
	throw std::runtime_error("fvmPutGlobalData failed!");
  }

  // clear shared memory again
  memset((int*)(fvmGetShmemPtr()), 0, test_alloc.size);

  // get memory back from allocation
  comm = fvmGetGlobalData(test_alloc
					   , 0
					   , test_alloc.size
					   , 0
					   , 0
  ...
  if (commState != COMM_HANDLE_OK)
  {
	MLOG(ERROR, "fvmGetGlobalData() failed: " << commState);
	throw std::runtime_error("fvmGetGlobalData failed!");
  }
  ...
  MLOG(INFO, "global allocation test successful");
...
SDPA_MOD_INIT_START(pc-test)
{
  ...
  SDPA_REGISTER_FUN_START(test_global_alloc);
    SDPA_ADD_INP( "size", std::size_t );
    SDPA_ADD_OUT( "error_code", int );
  SDPA_REGISTER_FUN_END(test_global_alloc);
}
