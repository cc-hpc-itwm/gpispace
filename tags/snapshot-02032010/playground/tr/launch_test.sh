...
job_id=$( sdpac submit ./remig.master.gwdl )
echo $job_id
start_time=$( date +%s )
echo -n "waiting for job finish..."
exit_code=2
while [ 0 -eq 0 ]
do
	sleep 1
	status=$( ./sdpac status $job_id )
	tmp=`echo $status | grep -q -i 'finished'`
	finished=$?
	tmp=`echo $status | grep -q -i 'failed'`
	failed=$?
	if [ $finished -eq 0 ]
	then
		echo "finished."
...
end_time=$( date +%s )
echo "Computation took $( expr $end_time - $start_time ) seconds!"
exit $exit_code
