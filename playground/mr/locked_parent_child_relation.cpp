// mirko.rahn@itwm.fraunhofer.de

#define BOOST_TEST_MODULE locked_parent_child_relation
#include <boost/test/unit_test.hpp>

#include <boost/bimap/bimap.hpp>
#include <boost/bimap/unordered_multiset_of.hpp>
#include <boost/bimap/unordered_set_of.hpp>
#include <boost/function.hpp>
#include <boost/optional.hpp>
#include <boost/thread.hpp>

#include <boost/foreach.hpp>
#include <boost/unordered_map.hpp>
#include <boost/unordered_set.hpp>

#include <fhg/util/boost/test/printer/set.hpp>
#include <fhg/util/now.hpp>

#include <set>

//! \todo add test using more than one thread

namespace
{
  namespace bbm = boost::bimaps;

  typedef int id_type;

  struct locked_parent_child_relation_bimap_type
  {
    void started (id_type parent, id_type child);
    bool terminated (id_type parent, id_type child);

    boost::optional<id_type> parent (id_type child) const;
    bool contains (id_type parent) const;

    void apply (id_type parent, boost::function<void (id_type)>) const;

  private:
    mutable boost::mutex _relation_mutex;
    typedef bbm::bimap
      < bbm::unordered_multiset_of<id_type> // parent can have many childs
      , bbm::unordered_set_of<id_type>      // child can have at most one parent
      , bbm::set_of_relation<>
      > relation_type;
    relation_type _relation;
  };

  void locked_parent_child_relation_bimap_type::started
    (id_type parent, id_type child)
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    // no child id will occur twice
    assert (_relation.right.find (child) == _relation.right.end());

    _relation.insert (relation_type::value_type (parent, child));
  }

  bool locked_parent_child_relation_bimap_type::terminated
    (id_type parent, id_type child)
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    assert (_relation.right.find (child) != _relation.right.end());

    _relation.erase (relation_type::value_type (parent, child));

    return !contains (parent);
  }

  boost::optional<id_type> locked_parent_child_relation_bimap_type::parent
    (id_type child) const
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    relation_type::right_map::const_iterator const pos
      (_relation.right.find (child));

    if (pos != _relation.right.end())
    {
      return pos->second;
    }

    return boost::none;
  }

  bool locked_parent_child_relation_bimap_type::contains (id_type parent) const
  {
    return _relation.left.find (parent) != _relation.left.end();
  }

  void locked_parent_child_relation_bimap_type::apply
    (id_type parent, boost::function<void (id_type)> fun) const
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    relation_type::left_map::const_iterator pos (_relation.left.find (parent));

    do
    {
      fun (pos->second);
      ++pos;
    } while (pos != _relation.left.end() && pos->first == parent);
  }

  struct locked_parent_child_relation_unordered_map_type
  {
    void started (id_type parent, id_type child);
    bool terminated (id_type parent, id_type child);

    boost::optional<id_type> parent (id_type child) const;
    bool contains (id_type parent) const;

    void apply (id_type parent, boost::function<void (id_type)>) const;

  private:
    mutable boost::mutex _relation_mutex;
    typedef boost::unordered_map<id_type, boost::unordered_set<id_type> >
    relation_type;
    relation_type _relation;
  } _running_jobs;

  void locked_parent_child_relation_unordered_map_type::started
  (id_type parent, id_type child)
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    _relation[parent].insert (child);
  }

  bool locked_parent_child_relation_unordered_map_type::terminated
  (id_type parent, id_type child)
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    assert (_relation.find (parent) != _relation.end());

    relation_type::iterator const childs (_relation.find (parent));

    childs->second.erase (child);

    if (childs->second.empty())
    {
      _relation.erase (childs);

      return true;
    }

    return false;
  }

  boost::optional<id_type>
  locked_parent_child_relation_unordered_map_type::parent (id_type child) const
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    BOOST_FOREACH (relation_type::value_type const& entry, _relation)
    {
      if (entry.second.count (child))
      {
        return entry.first;
      }
    }

    return boost::none;
  }

  bool locked_parent_child_relation_unordered_map_type::contains
  (id_type parent) const
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    return _relation.find (parent) != _relation.end();
  }

  void locked_parent_child_relation_unordered_map_type::apply
  (id_type parent, boost::function<void (id_type)> fun) const
  {
    boost::mutex::scoped_lock const _ (_relation_mutex);

    BOOST_FOREACH (id_type child, _relation.find (parent)->second)
    {
      fun (child);
    }
  }
}

namespace
{
  template<typename T>
  void _contains()
  {
    T relation;

    BOOST_REQUIRE (!relation.contains (0));
    BOOST_REQUIRE (!relation.contains (1));

    relation.started (0, 0);

    BOOST_REQUIRE (relation.contains (0));
    BOOST_REQUIRE (!relation.contains (1));

    relation.started (2, 2);

    BOOST_REQUIRE (relation.contains (0));
    BOOST_REQUIRE (!relation.contains (1));

    relation.started (1, 1);

    BOOST_REQUIRE (relation.contains (0));
    BOOST_REQUIRE (relation.contains (1));

    relation.terminated (1, 1);

    BOOST_REQUIRE (relation.contains (0));
    BOOST_REQUIRE (!relation.contains (1));

    relation.terminated (0, 0);

    BOOST_REQUIRE (!relation.contains (0));
    BOOST_REQUIRE (!relation.contains (1));
    BOOST_REQUIRE (relation.contains (2));
  }
}
BOOST_AUTO_TEST_CASE (contains)
{
  _contains<locked_parent_child_relation_unordered_map_type>();
  _contains<locked_parent_child_relation_bimap_type>();
}

namespace
{
  template<typename T>
  void _started_terminated()
  {
    T relation;

    relation.started (0, 0);
    BOOST_REQUIRE_EQUAL (relation.terminated (0, 0), true);

    relation.started (0, 0);
    relation.started (0, 1);

    BOOST_REQUIRE_EQUAL (relation.terminated (0, 0), false);
    BOOST_REQUIRE_EQUAL (relation.terminated (0, 1), true);

    relation.started (0, 0);
    relation.started (0, 1);
    relation.started (1, 2);
    relation.started (1, 3);

    BOOST_REQUIRE_EQUAL (relation.terminated (0, 1), false);
    BOOST_REQUIRE_EQUAL (relation.terminated (1, 3), false);
    BOOST_REQUIRE_EQUAL (relation.terminated (0, 0), true);
    BOOST_REQUIRE_EQUAL (relation.terminated (1, 2), true);
  }
}
BOOST_AUTO_TEST_CASE (started_terminated)
{
  _started_terminated<locked_parent_child_relation_unordered_map_type>();
  _started_terminated<locked_parent_child_relation_bimap_type>();
}

namespace
{
  template<typename T>
  void _parent()
  {
    T relation;

    BOOST_REQUIRE (!relation.parent (0));
    BOOST_REQUIRE (!relation.parent (1));
    BOOST_REQUIRE (!relation.parent (2));

    relation.started (0, 0);

    BOOST_REQUIRE (relation.parent (0));
    BOOST_REQUIRE (!relation.parent (1));
    BOOST_REQUIRE (!relation.parent (2));
    BOOST_REQUIRE_EQUAL (*relation.parent (0), 0);

    relation.started (0, 1);

    BOOST_REQUIRE (relation.parent (0));
    BOOST_REQUIRE (relation.parent (1));
    BOOST_REQUIRE (!relation.parent (2));
    BOOST_REQUIRE_EQUAL (*relation.parent (0), 0);
    BOOST_REQUIRE_EQUAL (*relation.parent (1), 0);

    relation.started (1, 2);

    BOOST_REQUIRE (relation.parent (0));
    BOOST_REQUIRE (relation.parent (1));
    BOOST_REQUIRE (relation.parent (2));
    BOOST_REQUIRE_EQUAL (*relation.parent (0), 0);
    BOOST_REQUIRE_EQUAL (*relation.parent (1), 0);
    BOOST_REQUIRE_EQUAL (*relation.parent (2), 1);

    relation.terminated (0, 1);

    BOOST_REQUIRE (relation.parent (0));
    BOOST_REQUIRE (!relation.parent (1));
    BOOST_REQUIRE (relation.parent (2));
    BOOST_REQUIRE_EQUAL (*relation.parent (0), 0);
    BOOST_REQUIRE_EQUAL (*relation.parent (2), 1);

    relation.terminated (0, 0);

    BOOST_REQUIRE (!relation.parent (0));
    BOOST_REQUIRE (!relation.parent (1));
    BOOST_REQUIRE (relation.parent (2));
    BOOST_REQUIRE_EQUAL (*relation.parent (2), 1);

    relation.terminated (1, 2);

    BOOST_REQUIRE (!relation.parent (0));
    BOOST_REQUIRE (!relation.parent (1));
    BOOST_REQUIRE (!relation.parent (2));
  }
}
BOOST_AUTO_TEST_CASE (parent)
{
  _parent<locked_parent_child_relation_unordered_map_type>();
  _parent<locked_parent_child_relation_bimap_type>();
}

namespace
{
  std::set<id_type> childs;

  void fun (id_type child)
  {
    childs.insert (child);
  }

  template<typename T>
  void _apply()
  {
#define APPLY(key)                              \
    childs.clear();                             \
    expected.clear();                           \
    relation.apply (key, fun)

    T relation;

    std::set<id_type> expected;

    relation.started (0, 0);

    APPLY (0);
    expected.insert (0);
    BOOST_REQUIRE_EQUAL (childs, expected);

    relation.started (1, 1);

    APPLY (0);
    expected.insert (0);
    BOOST_REQUIRE_EQUAL (childs, expected);

    APPLY (1);
    expected.insert (1);
    BOOST_REQUIRE_EQUAL (childs, expected);

    relation.started (0, 2);

    APPLY (0);
    expected.insert (0);
    expected.insert (2);
    BOOST_REQUIRE_EQUAL (childs, expected);

    relation.terminated (0, 0);

    APPLY (0);
    expected.insert (2);
    BOOST_REQUIRE_EQUAL (childs, expected);

#undef APPLY
  }
}
BOOST_AUTO_TEST_CASE (apply)
{
  _apply<locked_parent_child_relation_unordered_map_type>();
  _apply<locked_parent_child_relation_bimap_type>();
}

namespace
{
  template<typename T>
  double _performance_started (id_type num_parent, id_type num_child)
  {
    T relation;

    double t (-fhg::util::now());

    id_type child (0);

    for (id_type parent (0); parent < num_parent; ++parent)
    {
      for (id_type c (0); c < num_child; ++c, ++child)
      {
        relation.started (parent, child);
      }
    }

    return t + fhg::util::now();
  }
}
BOOST_AUTO_TEST_CASE (performance_started)
{
  BOOST_CHECK_LT
    ( _performance_started<locked_parent_child_relation_unordered_map_type>
      (10, 250000)
    , 1.0
    );
  BOOST_CHECK_LT
    ( _performance_started<locked_parent_child_relation_unordered_map_type>
      (2500, 2500)
    , 1.0
    );
  BOOST_CHECK_LT
    ( _performance_started<locked_parent_child_relation_unordered_map_type>
      (250000, 10)
    , 1.0
    );

  BOOST_CHECK_LT
    ( _performance_started<locked_parent_child_relation_bimap_type>
      (10, 125000)
    , 1.0
    );
  BOOST_CHECK_LT
    ( _performance_started<locked_parent_child_relation_bimap_type>
      (1250, 1250)
    , 1.0
    );
  BOOST_CHECK_LT
    ( _performance_started<locked_parent_child_relation_bimap_type>
      (125000, 10)
    , 1.0
    );
}

namespace
{
  template<typename T>
  double _performance_parent (id_type num_parent, id_type num_child)
  {
    T relation;

    id_type child (0);

    for (id_type parent (0); parent < num_parent; ++parent)
    {
      for (id_type c (0); c < num_child; ++c, ++child)
      {
        relation.started (parent, child);
      }
    }

    double t (-fhg::util::now());

    for (id_type c (0); c < child; ++c)
    {
      BOOST_REQUIRE (relation.parent (c));
    }

    return t + fhg::util::now();
  }
}
BOOST_AUTO_TEST_CASE (performance_parent)
{
  BOOST_CHECK_LT
    ( _performance_parent<locked_parent_child_relation_unordered_map_type>
      (10, 75000)
    , 1.0
    );
  BOOST_CHECK_LT
    ( _performance_parent<locked_parent_child_relation_unordered_map_type>
      (2500, 10)
    , 1.0
    );

  BOOST_CHECK_LT
    ( _performance_parent<locked_parent_child_relation_bimap_type>
      (10, 400000)
    , 1.0
    );
  BOOST_CHECK_LT
    ( _performance_parent<locked_parent_child_relation_bimap_type>
      (400000, 10)
    , 1.0
    );
}
