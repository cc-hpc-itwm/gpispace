...
static void receive_config_or_die();
...
int main (int ac, char **av)
{
  ...
  snprintf ( config.kvs_host
           , sizeof(config.kvs_host)
           , "localhost"
           );
  config.kvs_port = 2439;

  snprintf ( config.log_host
  ...
  if (is_master)
  {
    ...
    if (0 !=strlen (pidfile))
    {
      ...
      if (0 == pidfile_stream)
      {
        fprintf(stderr, "%s: could not open pidfile for writing: %s: %s\n", program_name, pidfile, strerror(errno));
        exit(EXIT_FAILURE);
      }
    }
...
static int check_gpi_on_node (const char * hostname)
{
  int error_code;
...
static void distribute_config_or_die ()
{
  ...
  for (size_t rank (1); rank < getNodeCountGPI(); ++rank)
  {
    ...
    {
      fprintf(stderr, "%s: could not send config to %lu\n", program_name, rank);
      killProcsGPI();
      exit(EXIT_FAILURE);
    }
  }
...
static void receive_config_or_die ()
{
  ...
  {
    fprintf( stderr
           , "%s: expected config from rank 0, got message from: %d\n"
           , program_name
           , source
           );
    exit(EXIT_FAILURE);
