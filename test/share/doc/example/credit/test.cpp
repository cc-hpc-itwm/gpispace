...
BOOST_AUTO_TEST_CASE (tutorial_work_and_wait)
{
  ...
  gspc::util::temporary_path const shared_directory
    (gspc::testing::shared_directory (vm) / "tutorial_work_and_wait");
...
BOOST_AUTO_TEST_CASE (tutorial_work_and_wait_credit)
{
  ::boost::program_options::options_description options_description;
  ...
  gspc::util::temporary_path const shared_directory
    (gspc::testing::shared_directory (vm) / "tutorial_work_and_wait_credit");
  ...
  std::multimap<std::string, gspc::pnet::type::value::value_type> const result
    ( gspc::client (drts).put_and_run
        (gspc::workflow (make.pnet()), {{"n", 12L}, {"c", 2L}})
    );
