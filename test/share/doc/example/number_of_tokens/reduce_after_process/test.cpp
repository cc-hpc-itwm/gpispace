...
BOOST_AUTO_TEST_CASE (number_of_tokens_reduce_after_process)
{
  ...
  gspc::scoped_runtime_system const drts
    ...
    , installation
    , fmt::format ( "worker:{}"
                  , gspc::testing::random<unsigned>{} (10U, 1U)
                  )
    , rifds.entry_points()
    };

  gspc::util::temporary_path const _pebbles_dir
    ( std::filesystem::path {shared_directory}
    / gspc::testing::unique_path()
    );
  std::filesystem::path const pebbles_dir {_pebbles_dir};

  auto const N {gspc::testing::random<unsigned long>{} (200UL, 1UL)};
