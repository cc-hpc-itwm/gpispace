...
#include <gspc/testing/measure_average_time.hpp>
...
#define service_and_client_setup_with_blob_size(size)                     \
  ...
  gspc::rpc::service_dispatcher service_dispatcher;                        \
                                                                          \
  gspc::rpc::service_handler<protocol::get_blob> get_blob_service          \
    {service_dispatcher, [&] { return blob; }};                           \
  gspc::rpc::service_handler<protocol::put_blob> put_blob_service          \
    {service_dispatcher, [] (std::vector<char>) {}};                      \
  gspc::rpc::service_handler<protocol::echo_blob> echo_blob_service        \
  ...
  gspc::rpc::remote_tcp_endpoint endpoint                                  \
    ...
    , gspc::util::connectable_to_address_string (server.local_endpoint())  \
                                                                          ...
                                                                          \
  gspc::rpc::sync_remote_function<protocol::get_blob> get_blob {endpoint}; \
  gspc::rpc::sync_remote_function<protocol::put_blob> put_blob {endpoint}; \
  gspc::rpc::sync_remote_function<protocol::echo_blob> echo_blob {endpoint}

#define require_blob_functions_le(limit, repetitions)                     \
  BOOST_REQUIRE_LE                                                        \
    ( gspc::testing::measure_average_time<std::chrono::microseconds> \
...
#define multiple_clients_with_one_server_setup( server_io_service_threads \
  ...
  gspc::util::scoped_boost_asio_io_service_with_threads server_io_service  \
    {server_io_service_threads};                                          \
  gspc::util::scoped_boost_asio_io_service_with_threads client_io_service  \
    {client_io_service_threads};                                          \
                                                                          \
  gspc::rpc::service_dispatcher service_dispatcher                         \
    {gspc::util::serialization::exception::serialization_functions()};     \
  gspc::rpc::service_handler<protocol::nop> nop_service                    \
    {service_dispatcher, []{}};                                           \
  gspc::rpc::service_tcp_provider const server                             \
  ...
  unsigned short const endpoint_port (server.local_endpoint().port());    \
                                                                          \
  std::list<gspc::rpc::remote_tcp_endpoint> endpoints;                     \
  std::vector<gspc::rpc::remote_function<protocol::nop>> functions;        \
