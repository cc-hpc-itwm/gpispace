...
{
  std::string const global (gspc::testing::random_content_string());
  std::string const local (gspc::testing::random_content_string());
...
{
  check_memory_put_is_stored_in_function (std::nullopt);
...
{
  check_memory_getput_is_stored_in_function (std::nullopt);
...
<defun name="{0}">
  ...
  <{1}/>
    ...
    gspc::testing::require_exception_with_message
      <gspc::xml::parse::error::memory_transfer_for_non_module>
      ( [&state, &input]()
      { std::istringstream input_stream (input);
        gspc::xml::parse::just_parse (state, input_stream);
      }
