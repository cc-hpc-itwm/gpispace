...
BOOST_AUTO_TEST_CASE (xml_parse_valid_preferences_and_match_targets_in_function)
{
  ...
  std::istringstream input_stream (multi_mod.pnet.pnet_xml);
  gspc::xml::parse::type::function_type fun
    = gspc::xml::parse::just_parse (state, input_stream);

  std::list<gspc::xml::parse::type::preference_type> const& out_targets
...
BOOST_AUTO_TEST_CASE (xml_parse_valid_preferences_and_propagate_to_we)
{
  ...
  auto const transition (gspc::xml::parse::xml_to_we (fun, state));
...
BOOST_AUTO_TEST_CASE (xml_parse_valid_preferences_and_generate_cpp)
{
  ...
  std::filesystem::path const path
    ( gspc::util::executable_path().parent_path().string()
      + "/gen_modules_with_preferences_test"
    );
  state.path_to_cpp() = path.string();

  BOOST_REQUIRE_NO_THROW (gspc::xml::parse::generate_cpp (fun, state));

  std::filesystem::path const makefile_path
    (path.string() + "/Makefile");
  std::string const makefile
    {gspc::util::read_file<std::string> (makefile_path)};
  ...
  for (auto const& target : multi_mod.test_targets)
  {
    ...
    std::filesystem::path const wrapper_cpp
      (path.string() + "/pnetc/op/" + mod_name + ".cpp");

    std::filesystem::path const func_def_cpp
      (path.string() + "/pnetc/op/" + mod_name + "/func.cpp");

    std::filesystem::path const func_def_hpp
    ...
    {
      BOOST_REQUIRE (std::filesystem::exists (wrapper_cpp));
      BOOST_REQUIRE (std::filesystem::exists (func_def_cpp));
      BOOST_REQUIRE (std::filesystem::exists (func_def_hpp));
    }
    ...
    std::string const wrapper_cpp_content
      {gspc::util::read_file<std::string> (wrapper_cpp)};
