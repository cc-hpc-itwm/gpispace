...
BOOST_AUTO_TEST_CASE (xml_transport_bytearray)
{
  ...
  gspc::util::temporary_path const shared_directory
    (gspc::testing::shared_directory (vm) / "xml_transport_bytearray");
  ...
  gspc::pnet::type::value::value_type const point
    (gspc::pnet::type::value::read ("Struct [x := 1.0, y := 2.0]"));
  ...
  std::multimap<std::string, gspc::pnet::type::value::value_type> const input
    {{"point", point}};

  auto const output
    (gspc::client (drts).put_and_run (gspc::workflow (make.pnet()), input));

  GSPC_TESTING_REQUIRE_CONTAINER_IS_PERMUTATION (input, output);
}
