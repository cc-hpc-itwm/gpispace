...
#include <boost/test/unit_test.hpp>
  ...
  namespace gspc::logging
  {
    ...
    static
    bool operator== ( std::variant<tcp_endpoint, socket_endpoint> const& lhs
                    , socket_endpoint const& rhs
                    )
    {
      return std::holds_alternative<socket_endpoint> (lhs)
        && std::get<socket_endpoint> (lhs) == rhs;
    }
    static
    bool operator== ( std::variant<tcp_endpoint, socket_endpoint> const& lhs
    ...
    {
      return std::holds_alternative<tcp_endpoint> (lhs)
        && std::get<tcp_endpoint> (lhs) == rhs;
    }

    static
    std::ostream& operator<< ( std::ostream& os
                             , std::variant<tcp_endpoint, socket_endpoint> const& v
                             )
    {
      return std::visit
        ( [&] (auto const& endpoint) -> std::ostream&
          {
    ...
    namespace
    {
      std::string random_host()
      {
        ...
        do
        {
          ...
          result = gspc::testing::random_string_without (":>");
        }
        while (result.empty());
        return result;
      }
      ...
      {
        return {random_host(), gspc::testing::random<unsigned short>{}()};
      }
      socket_endpoint random_socket_endpoint()
      {
        ...
        {
          // '>': end of block in surrounding endpoint's parser
          path = gspc::testing::random_string_without (">");
        }
