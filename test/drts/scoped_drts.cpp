// Copyright (C) 2015-2016,2018-2021,2023-2026 Fraunhofer ITWM
  ...
  )
{
  ...
  gspc::util::temporary_path const shared_directory
    (gspc::testing::shared_directory (vm) / "drts_scoped_drts");
      ...
      ([&info_output, &hosts]()
       {
         ...
         BOOST_REQUIRE
           ( std::regex_match
             ...
             , match
             , std::regex
               ("I: starting base scheduler components on ((.+) [0-9]+ [0-9]+)...")
             )
           );
