...
BOOST_AUTO_TEST_CASE (use_fixed_ports_for_agents_and_workers)
{
  ...
  std::unique_ptr<gspc::testing::hopefully_free_port> agent_port;
  std::unique_ptr<gspc::testing::hopefully_free_port> worker_port;
  ...
  while (!drts)
  {
    try
    {
      agent_port = std::make_unique<gspc::testing::hopefully_free_port>();
      worker_port = std::make_unique<gspc::testing::hopefully_free_port>();
  ...
  BOOST_REQUIRE (is_using_port ("gspc-agent", *agent_port));
