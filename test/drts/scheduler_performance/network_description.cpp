...
#include <fmt/core.h>
  ...
  namespace drts::test
  {
    std::string create_network_description
      ...
      , std::optional<std::string> const& with_num_worker_prop
      , std::optional<std::string> const& with_memory_buffers
      )
    {
      return fmt::format (R"EOS(
                ...
                <transition name="compute_A">
                  <defun>
                    ...
                    <module name="module_{0}"
                      ...
                      <code><![CDATA[
                        {4}
                        task_finished = gspc::we::type::literal::control();
                      ]]></code>
                    </module>
                  </defun>
        ...
        , name
        , with_num_worker_prop.value_or ("")
        , with_memory_buffers.value_or ("")
