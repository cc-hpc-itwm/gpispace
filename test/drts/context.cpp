...
namespace
{
  ...
  struct channel_from_child_to_parent
  {
    ...
    {
      gspc::util::syscall::pipe (_fds.both);
    }
    ...
    {
      BOOST_REQUIRE_EQUAL
        (sizeof (x), gspc::util::syscall::write (_fds.fd.write, &x, sizeof (x)));
    }
    template<typename T>
      T read() const
    {
      ...
      BOOST_REQUIRE_EQUAL
        (sizeof (x), gspc::util::syscall::read (_fds.fd.read, &x, sizeof (x)));
    ...
    context_fixture()
      ...
      , context
        ( drts::worker::context_constructor
          (gspc::testing::random_string(), {}, logger)
        )
    {
...
{
  int const r {gspc::testing::random<int>()()};
  ...
  (execute_and_kill_on_cancel_DO_NOT_OUTPUT_TO_STANDARD_STREAMS_FROM_WITHIN_calls_on_signal, context_fixture)
{
  ...
  context.execute_and_kill_on_cancel_DO_NOT_OUTPUT_TO_STANDARD_STREAMS_FROM_WITHIN
    ( []()
      {
        gspc::util::syscall::kill (gspc::util::syscall::getpid(), SIGUSR1);
      }
    , &on_cancel_unexpected
  ...
  (execute_and_kill_on_cancel_DO_NOT_OUTPUT_TO_STANDARD_STREAMS_FROM_WITHIN_calls_SIGINT_blocked, context_fixture)
{
  ...
  context.execute_and_kill_on_cancel_DO_NOT_OUTPUT_TO_STANDARD_STREAMS_FROM_WITHIN
    ( [] ()
      {
        gspc::util::syscall::kill (gspc::util::syscall::getpid(), SIGINT);
      }
    , &on_cancel_unexpected
    , &on_signal_unexpected
    , [&exited] (int exit_code)
...
{
  std::runtime_error const exception {gspc::testing::random_string()};
