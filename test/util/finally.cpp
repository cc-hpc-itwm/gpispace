...
BOOST_AUTO_TEST_CASE (finally_is_called_on_scope_exit)
{
  ...
  {
    gspc::util::finally_t<std::function<void()>> const f0
      ([&stack] { stack.push (0); });

    {
      gspc::util::finally_t<std::function<void()>> const f1
        ([&stack] { stack.push (1); });
...
#define TEST_FINALLY(fun)                              \
  gspc::util::finally_t<std::function<void()>> const    \
...
BOOST_AUTO_TEST_CASE (misuse_move_ctor)
{
  ...
  {
    gspc::util::finally_t<std::function<void()>> f0
      ([&stack] { stack.push (0); });

    gspc::util::finally_t<std::function<void()>> const f1
      ([&stack] { stack.push (1); });

    gspc::util::finally_t<std::function<void()>> const f2
      (std::move (f0));
  }
