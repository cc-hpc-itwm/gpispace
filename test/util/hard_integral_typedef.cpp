...
FHG_UTIL_HARD_INTEGRAL_TYPEDEF_SERIALIZATION (gspc::util::test_t)
...
#define REQUIRE(...) do { auto check = __VA_ARGS__; BOOST_REQUIRE (check); } while (false)
    ...
    BOOST_AUTO_TEST_CASE (auto_generated_operations)
    {
      ...
      {
        std::size_t value (gspc::testing::random<std::size_t>{}());
        test_t t {value};
      ...
      GSPC_TESTING_REQUIRE_SERIALIZED_TO_ID
        ({gspc::testing::random<test_t>{}()}, test_t);
    ...
    {
      GSPC_TESTING_COMPILETIME_REQUIRE_EQUAL (test_t{}, test_t {0});
      GSPC_TESTING_COMPILETIME_REQUIRE_NE (test_t {2}, test_t {1});
      GSPC_TESTING_COMPILETIME_REQUIRE_LT (test_t {0}, test_t {1});
      GSPC_TESTING_COMPILETIME_REQUIRE_LE (test_t {0}, test_t {1});
      GSPC_TESTING_COMPILETIME_REQUIRE_GT (test_t {2}, test_t {1});
      GSPC_TESTING_COMPILETIME_REQUIRE_GE (test_t {1}, test_t {1});
      GSPC_TESTING_COMPILETIME_REQUIRE (!test_t {0});
    }
...
FHG_UTIL_HARD_INTEGRAL_TYPEDEF_HASH (gspc::util::hashable_t)
  ...
  namespace gspc::util
  {
    ...
    {
      auto value (gspc::testing::random<hashable_t>{}());
...
FHG_UTIL_HARD_INTEGRAL_TYPEDEF_NUMERIC_LIMITS (gspc::util::limitable_t)
FHG_UTIL_HARD_INTEGRAL_TYPEDEF_NUMERIC_LIMITS (gspc::util::signed_limitable_t)
GSPC_HARD_INTEGRAL_TYPEDEF_LOG_VALUE_PRINTER (gspc::util::limitable_t)
