...
#include <gspc/util/qt/qdatastream_enum.hpp>
...
#define IS_VALID_EXPRESSION_TRAIT(name_, expected_ret_, expression_...) \
  ...
  template<typename T>                                                  \
    struct name_<T, gspc::util::cxx17::void_t<decltype (expression_)>>   \
...
{
  struct S {};
  GSPC_TESTING_COMPILETIME_CHECK (!has_qdatastream_in<S>{});
  GSPC_TESTING_COMPILETIME_CHECK (!has_qdatastream_out<S>{});

  using V = void;
  GSPC_TESTING_COMPILETIME_CHECK (!has_qdatastream_in<V>{});
  GSPC_TESTING_COMPILETIME_CHECK (!has_qdatastream_out<V>{});
}

BOOST_AUTO_TEST_CASE (exists_for_enum)
{
  enum E {};
  GSPC_TESTING_COMPILETIME_CHECK (has_qdatastream_in<E>{});
  GSPC_TESTING_COMPILETIME_CHECK (has_qdatastream_out<E>{});

  enum class EC {};
  GSPC_TESTING_COMPILETIME_CHECK (has_qdatastream_in<EC>{});
  GSPC_TESTING_COMPILETIME_CHECK (has_qdatastream_out<EC>{});
}
...
namespace n1
{
  ...
  namespace n4
  {
    ...
    {
      GSPC_TESTING_COMPILETIME_CHECK (has_qdatastream_in<n2::E>{});
      GSPC_TESTING_COMPILETIME_CHECK (has_qdatastream_out<n2::E>{});
    }
  }
...
{
  GSPC_TESTING_COMPILETIME_CHECK (has_qdatastream_in<n1::n2::E>{});
  GSPC_TESTING_COMPILETIME_CHECK (has_qdatastream_out<n1::n2::E>{});
}
...
GSPC_BOOST_TEST_LOG_VALUE_PRINTER (EC_16, os, val)
{
