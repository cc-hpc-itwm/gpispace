...
{
  gspc::util::latch (0).wait();
}

BOOST_AUTO_TEST_CASE (counting_when_reached_zero_is_logic_error)
{
  gspc::testing::require_exception
    ( [] { gspc::util::latch (0).count_down(); }
    , gspc::util::error::count_down_for_zero()
    );
}

BOOST_AUTO_TEST_CASE (one_thread_waits_the_other_counts_down)
{
  gspc::util::latch latch (1);

  std::thread waiter (&gspc::util::latch::wait, &latch);
  std::thread counter (&gspc::util::latch::count_down, &latch);
...
BOOST_AUTO_TEST_CASE (many_counters)
{
  ...
  gspc::util::latch latch (num_threads);

  std::vector<std::thread> counters;
  while (num_threads --> 0)
  {
    counters.emplace_back (&gspc::util::latch::count_down, &latch);
  }
...
BOOST_AUTO_TEST_CASE (many_waiters)
{
  ...
  while (num_threads --> 0)
  {
    waiters.emplace_back (&gspc::util::latch::wait, &latch);
  }
...
namespace
{
  ...
  private:
    gspc::util::latch _notify;
    gspc::util::latch _release;
  };
}
...
BOOST_AUTO_TEST_CASE (two_latches_form_a_startup_barrier)
{
  ...
  std::unique_ptr<gspc::util::latch> counter;
  ...
  barrier.wait();
  counter = std::make_unique<gspc::util::latch> (num_threads);
  barrier.release();
...
BOOST_AUTO_TEST_CASE (wait_and_reset_allows_for_reuse)
{
  ...
  namespace gspc::util
  {
      ...
      )
    {
      ...
      latch* latch_ {reinterpret_cast<latch*> (&storage)};
