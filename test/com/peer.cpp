// Copyright (C) 2010,2013-2026 Fraunhofer ITWM
  ...
  (peer_does_not_hang_when_resolve_throws, certificates_data, certificates)
{
  gspc::testing::require_exception
    ( [&certificates]
      {
        gspc::com::peer_t ( std::make_unique<::boost::asio::io_service>()
                         , gspc::com::host_t ("NONONONONONONONONONO")
                         , gspc::com::port_t (0)
                         , certificates
                         );
      }
...
namespace
{
  gspc::com::host_t host (::boost::asio::ip::tcp::endpoint const& ep)
  {
    return gspc::com::host_t
      (gspc::util::connectable_to_address_string (ep.address()));
  }
  gspc::com::port_t port (::boost::asio::ip::tcp::endpoint const& ep)
  {
    return gspc::com::port_t (ep.port());
  }
                     ...
                     )
{
  ...
  std::promise<gspc::com::peer_t::Received> recv_finished;
  ...
  std::optional<p2p::address_t> connection;
  auto const connect
    ( [&]
      {
  ...
  auto const send
    ( [&]
      {
        peer_1.send (connection.value(), message);
      }
    );
  ...
  BOOST_REQUIRE_EQUAL (error, ::boost::system::error_code{});
  ...
  namespace gspc::com
  {
                         ...
                         )
    {
      ...
      BOOST_REQUIRE_EQUAL (received.ec(), ::boost::system::error_code{});
...
BOOST_DATA_TEST_CASE (peer_loopback_forbidden, certificates_data, certificates)
{
  ...
  gspc::testing::require_exception
    ( [&]
      {
        peer_1.connect_to ( gspc::com::host_t (gspc::util::hostname())
...
{
  auto const payload (gspc::testing::random<std::string>{}());

  using Received
    = std::variant<::boost::system::error_code, gspc::com::message_t>;
...

  {
    ...
    std::function<void (gspc::com::peer_t::Received)> record_receive;

    gspc::com::peer_t parent
      ( std::make_unique<::boost::asio::io_service>()
      , gspc::com::host_t ("localhost")
      , gspc::com::port_t (0)
      , certificates
  ...
  for (auto const& result : receive_results)
  {
    gspc::util::visit
      ...
      , [&] (gspc::com::message_t const& message)
        {
...
BOOST_AUTO_TEST_CASE (require_certificates_location_to_exist)
{
  ...
  gspc::testing::temporary_path const empty_directory;
  ...
  BOOST_REQUIRE_EXCEPTION
    ( peer_t peer_1 ( std::make_unique<::boost::asio::io_service>()
                    ...
                    , port_t (0)
                    , gspc::Certificates
                        {std::filesystem::path (empty_directory) / "nonexist"}
                    )
...
BOOST_AUTO_TEST_CASE (using_empty_Certificates_throws)
{
  ...
  gspc::testing::temporary_path const certificates_directory;

  gspc::testing::temporary_path const crt
    (std::filesystem::path (certificates_directory) / "server.crt");

  gspc::testing::temporary_path const key
    (std::filesystem::path (certificates_directory) / "server.key");
