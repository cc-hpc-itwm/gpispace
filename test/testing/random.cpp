      ...
      namespace
      {
        ...
        template<typename T>
          struct random_exists_and_is_callable
            <T, ::gspc::util::cxx17::void_t<decltype (std::declval<T&>() = random<T>{}())>>
              : std::true_type {};
...
FHG_UTIL_MAKE_COMBINED_STD_HASH
  (gspc::testing::not_so_random_value_type, v, v.value)
GSPC_BOOST_TEST_LOG_VALUE_PRINTER
  (gspc::testing::not_so_random_value_type, os, v)
{
  os << v.value;
}
