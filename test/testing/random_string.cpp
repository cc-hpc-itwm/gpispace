...
BOOST_AUTO_TEST_CASE (char_of_empty)
{
  gspc::testing::require_exception
    ( [] { gspc::testing::random<char>{} (std::string()); }
...
{
  BOOST_REQUIRE_EQUAL (gspc::testing::random<char>{} ("c"), 'c');
}
...
BOOST_AUTO_TEST_CASE (char_of_some)
{
  ...
  {
    BOOST_REQUIRE_NE
      ( chars.find (gspc::testing::random<char>{} (chars))
      , std::string::npos
      );
  }
}
...
BOOST_AUTO_TEST_CASE (char_of_distribution)
{
  ...
  {
    ++count[gspc::testing::random<char>{} (chars) - '0'];
    ++len;
  }
...
{
  gspc::testing::require_exception
    ( [] { gspc::testing::random<std::string>{} (std::string()); }
...
BOOST_AUTO_TEST_CASE (string_of_one)
{
  ...
  for (char const& c : gspc::testing::random<std::string>{} (chars))
  {
...
BOOST_AUTO_TEST_CASE (string_of_distribution)
{
  ...
  while (count[0] == 0 || len < (1UL << 15))
  {
    std::string const s (gspc::testing::random<std::string>{} (chars));
...
{
  auto const except ( gspc::testing::random<std::string>{}
