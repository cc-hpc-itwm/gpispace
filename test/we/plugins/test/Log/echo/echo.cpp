...
BOOST_AUTO_TEST_CASE (plugin_echo)
{
  ...
  gspc::util::temporary_path const shared_directory
    (gspc::testing::shared_directory (vm) / "plugin_echo");
  ...
  gspc::testing::make_net const make (installation, "echo", gspc::testing::source_directory (vm) );
  ...
  gspc::testing::unique_random<long> unique_random_long;

  gspc::pnet::type::value::value_type const before (unique_random_long());
  gspc::pnet::type::value::value_type const after (unique_random_long());
  ...
  GSPC_TESTING_REQUIRE_CONTAINER_IS_PERMUTATION
    (expected, gspc::util::read_lines (log_path));
}
