// Copyright (C) 2013-2026 Fraunhofer ITWM
...
struct fixture_scheduler_and_requirements_and_preferences
{
  ...
  fixture_scheduler_and_requirements_and_preferences()
    ...
    , _access_allocation_table
        (dynamic_cast<gspc::scheduler::daemon::CoallocationScheduler*> (_scheduler.get())->strategy_TESTING_ONLY())
  {}
...
namespace
{
  ...
  const std::optional<unsigned long> no_max_num_retries;
  ...
  gspc::we::type::Requirements_and_preferences require (unsigned long workers)
  {
    return { {}
           , {workers, no_max_num_retries}
           , gspc::we::type::null_transfer_cost
...
{
  add_worker (_worker_manager, "A0", {gspc::scheduler::Capability ("A")});
  add_worker (_worker_manager, "B0", {gspc::scheduler::Capability ("B")});
  add_worker (_worker_manager, "A1", {gspc::scheduler::Capability ("A")});
  add_worker (_worker_manager, "B1", {gspc::scheduler::Capability ("B")});
    ...
    )
  {
    gspc::scheduler::worker_id_t assigned_worker_with_max_cost
      (*std::max_element ( assigned_workers.begin()
                         ...
                         , [cost] ( gspc::scheduler::worker_id_t const& left
    ...
    for (gspc::scheduler::worker_id_t const& wid : worker_ids)
    {
                        ...
                        )
{
  ...
  const std::map<gspc::scheduler::worker_id_t, double>
  ...
  auto const _scheduler
    ( std::make_unique<gspc::scheduler::daemon::CoallocationScheduler>
        ( [&test_transfer_cost, n_req_workers] (gspc::scheduler::job_id_t const&)
          {
  ...
  for (gspc::scheduler::worker_id_t const& worker_id : worker_ids)
  {
    std::lock_guard<std::mutex> lock (worker_manager._mutex);
    worker_manager.add_worker
      ( worker_id
...
BOOST_AUTO_TEST_CASE (scheduling_bunch_of_jobs_with_preassignment_and_load_balancing)
{
  ...
  std::vector<gspc::scheduler::worker_id_t> worker_ids (n_workers);
  ...
  auto const _scheduler
    ( std::make_unique<gspc::scheduler::daemon::CoallocationScheduler>
        ( [&test_transfer_cost, &_computational_cost, n_req_workers]
          (gspc::scheduler::job_id_t const&)
          {
            return gspc::we::type::Requirements_and_preferences
              ( {}
  ...
  auto const allocation_table
    (gspc::scheduler::daemon::access_allocation_table_TESTING_ONLY (_scheduler.get()->strategy_TESTING_ONLY()));
  ...
  (request_arbitrary_number_of_workers, fixture_add_new_workers)
{
  ...
  while (n_assigned_workers < n_workers)
  {
    auto const n_req_workers
      (gspc::testing::random<unsigned int>{} (n_max_workers_per_job, 1));
  ...
  )
{
  ...
  std::set<gspc::scheduler::job_id_t> running_jobs;
  while (!assignment.empty())
  {
    ...
    if (!running_jobs.empty())
    {
      auto const n_finishing_jobs
        (gspc::testing::random<std::size_t>{} (running_jobs.size(), 1));
  ...
  )
{
  ...
  auto const num_workers
    (gspc::testing::random<unsigned int>{} (4, 2));
