...
#define SERVE_JOB_AND_CHECK_EXPECTED(job)     \
  [&job]                                      \
  ( gspc::scheduler::daemon::WorkerSet const&            \
  , gspc::scheduler::daemon::Implementation const&       \
  , gspc::scheduler::job_id_t const& started_job         \
  , std::function<gspc::com::p2p::address_t    \
      (gspc::scheduler::worker_id_t const&)>             \
  )                                           \
  {                                           \
...
namespace
{
  ...
  using set_workers_t = std::set<gspc::scheduler::worker_id_t>;
  using set_jobs_t = std::set<gspc::scheduler::job_id_t>;
}
  ...
  namespace gspc::scheduler::daemon
  {
    ...
    public:
      ...
      gspc::scheduler::daemon::WorkerSet workers (gspc::scheduler::job_id_t const& job) const
      {
        return _allocation_table.at (job)->workers();
      }

      gspc::scheduler::daemon::Implementation implementation (gspc::scheduler::job_id_t const& job) const
      {
...
struct fixture_scheduler_and_requirements_and_preferences
{
  ...
  gspc::scheduler::daemon::CostAwareWithWorkStealingStrategy& _strategy;
  gspc::scheduler::daemon::access_allocation_table_TESTING_ONLY _access_allocation_table;

  std::map<gspc::scheduler::job_id_t, std::set<gspc::scheduler::worker_id_t>> get_current_assignment() const
  {
  ...
  gspc::testing::unique_random<gspc::scheduler::job_id_t> job_ids;
  gspc::scheduler::job_id_t add_and_enqueue_job
    (gspc::we::type::Requirements_and_preferences reqs_and_prefs)
  {
  ...
  std::map
    < gspc::scheduler::job_id_t
    , gspc::we::type::Requirements_and_preferences
    > _requirements_and_preferences
    ;

  long count_assigned_jobs
    ( std::map<gspc::scheduler::job_id_t, std::set<gspc::scheduler::worker_id_t>> assignment
    , gspc::scheduler::worker_id_t const& worker_id
    )
  {
    auto value = [](std::pair<gspc::scheduler::job_id_t, std::set<gspc::scheduler::worker_id_t>>const& p)
                   {
                     return p.second;
                   };
    ...
    return (std::count_if ( ::boost::make_transform_iterator (assignment.begin(), value)
                          ...
                          , [&worker_id] (std::set<gspc::scheduler::worker_id_t> const& v)
                            {
  ...
  std::set<gspc::scheduler::job_id_t> assigned_tasks (gspc::scheduler::worker_id_t const& worker)
  {
    std::set<gspc::scheduler::job_id_t> tasks;
  ...
  std::set<gspc::scheduler::job_id_t> all_assigned_tasks()
  {
    std::set<gspc::scheduler::job_id_t> tasks;
    for (auto const& task_and_workers : get_current_assignment())
    {
  ...
  void require_worker_and_implementation
    ( gspc::scheduler::job_id_t const& job
    , gspc::scheduler::worker_id_t const& worker
    , std::optional<std::string> const& impl
    )
  {
    auto const assignment (get_current_assignment());
    BOOST_REQUIRE (assignment.count (job));

    BOOST_REQUIRE_EQUAL (gspc::scheduler::daemon::WorkerSet {worker}, workers (job));
  ...
  void finish_tasks_and_steal_work_when_idle_workers_exist
    ...
    , std::vector<gspc::scheduler::worker_id_t> const& test_workers
    )
  {
    unsigned int remaining_tasks (num_tasks);

    std::vector<gspc::scheduler::job_id_t> running_tasks;
    ...
    while (remaining_tasks > 0)
    {
      ...
      std::set<gspc::scheduler::job_id_t> started_tasks;
      _scheduler->start_pending_jobs
      ( [this, &started_tasks]
        ( gspc::scheduler::daemon::WorkerSet const& assigned_workers
        , gspc::scheduler::daemon::Implementation const& implementation
        , gspc::scheduler::job_id_t const& task
        , std::function<gspc::com::p2p::address_t (gspc::scheduler::worker_id_t const&)>
        )
        {
...
namespace
{
  ...
  gspc::we::type::Requirements_and_preferences require (std::string name_1)
  {
    return { {gspc::we::type::Requirement (name_1)}
  ...
  gspc::we::type::Requirements_and_preferences no_requirements_and_preferences()
  {
    return { {}
           , gspc::we::type::schedule_data()
           , gspc::we::type::null_transfer_cost
  ...
  void add_worker ( gspc::scheduler::daemon::WorkerManager& worker_manager
  ...
  void add_worker ( gspc::scheduler::daemon::WorkerManager& worker_manager
                  ...
                  , gspc::com::p2p::address_t const& address
                  )
  {
    std::lock_guard<std::mutex> lock (worker_manager._mutex);
    worker_manager.add_worker
...
MULTIPLE_FIXTURES_TEST_CASE
  ...
  , F
  , fixture_scheduler_and_requirements_and_preferences
      <gspc::scheduler::daemon::SingleAllocationScheduler>
  , fixture_scheduler_and_requirements_and_preferences
      <gspc::scheduler::daemon::CoallocationScheduler>
  )
{
...
MULTIPLE_FIXTURES_TEST_CASE
  ...
  , fixture_scheduler_and_requirements_and_preferences
      <gspc::scheduler::daemon::CoallocationScheduler>
  , fixture_scheduler_and_requirements_and_preferences
      <gspc::scheduler::daemon::GreedyScheduler>
  )
{
  add_worker (F::_worker_manager, "worker_0");
  ...
  )
{
  ...
  gspc::scheduler::job_id_t job_assigned_to_worker_0
...
struct serve_job_and_check_for_minimal_cost_assignement
{
  serve_job_and_check_for_minimal_cost_assignement()
    : _worker_ids
        ( generate_worker_names
            (gspc::testing::random<unsigned int>{} (20, 10))
        )
    ...
    , _scheduler
       ( std::make_unique<Scheduler>
           ( [this] (gspc::scheduler::job_id_t const&)
             {
               return gspc::we::type::Requirements_and_preferences
                 ( {}
                 , gspc::we::type::schedule_data()
                 , _transfer_cost
                 , computational_cost
  ...
  std::map<gspc::scheduler::worker_id_t, double>
    generate_costs (std::vector<std::string> worker_ids)
  {
    std::normal_distribution<> dist (0., 1.);

    std::map<gspc::scheduler::worker_id_t, double> map_costs;
    for (gspc::scheduler::worker_id_t const& worker : worker_ids)
    {
      map_costs.emplace
        (worker, dist (gspc::testing::detail::GLOBAL_random_engine()));
    }
  ...
  {
    gspc::scheduler::worker_id_t assigned_worker_with_max_cost
      ( *std::max_element
          ( assigned_workers.begin()
          , assigned_workers.end()
          , [this] ( gspc::scheduler::worker_id_t const& left
                   , gspc::scheduler::worker_id_t const& right
                   )
            { return _transfer_cost (left) < _transfer_cost (right); }
          )
      );

    for (gspc::scheduler::worker_id_t const& wid : _worker_ids)
    {
  ...
  std::map<gspc::scheduler::worker_id_t, double> _map_transfer_costs;
  ...
  )
{
  ...
  for (gspc::scheduler::worker_id_t const& worker_id : F::_worker_ids)
  {
    add_worker
      ( F::_worker_manager
      , worker_id
...
struct fixture_preassignment_and_load_balancing
{
  fixture_preassignment_and_load_balancing()
    ...
    , _host_ids (_n_hosts)
    , _dist (1.0, _n_hosts)
    , _rand_engine (gspc::testing::detail::GLOBAL_random_engine())
  ...
  std::vector<gspc::scheduler::worker_id_t> _worker_ids;
  unsigned int _n_hosts;
  std::vector<gspc::scheduler::worker_id_t> _host_ids;
  std::uniform_real_distribution<double> _dist;
  gspc::testing::detail::RandomNumberEngine& _rand_engine;
  ...
  )
{
  ...
  std::set<gspc::scheduler::worker_id_t> set_0 {"worker_0"};
  std::set<gspc::scheduler::worker_id_t> set_1 {"worker_1"};
  ...
  )
{
  ...
  std::string const name_capability {gspc::testing::random_string()};
  add_worker (F::_worker_manager, name_worker_0);
  add_worker ( F::_worker_manager
             , name_worker_1
             , {gspc::scheduler::Capability (name_capability)}
             );
  ...
  BOOST_REQUIRE_EQUAL
    (assignment.at (job_id), std::set<gspc::scheduler::worker_id_t> {name_worker_0});
}
  ...
  )
{
  ...
  std::set<gspc::scheduler::worker_id_t> const expected_assignment {name_worker_1};
  ...
  std::set<gspc::scheduler::job_id_t> jobs;

  for ( unsigned long k = 0
      ; k < gspc::testing::random<std::size_t>{} (100, 3)
      ; ++k
      )
  {
    jobs.emplace
  ...
  )
{
  ...
  add_worker
    (F::_worker_manager, "worker_0", {gspc::scheduler::Capability ("A")});
  add_worker
    (F::_worker_manager, "worker_1", {gspc::scheduler::Capability ("A")});
  ...
  add_worker
    (F::_worker_manager, "worker_2", {gspc::scheduler::Capability ("A")});
...

  {
    ...
    BOOST_REQUIRE (  assignment.at (job_0) == std::set<gspc::scheduler::worker_id_t>({"worker_2"})
                  || assignment.at (job_1) == std::set<gspc::scheduler::worker_id_t>({"worker_2"})
                  || assignment.at (job_2) == std::set<gspc::scheduler::worker_id_t>({"worker_2"})
                  );
  ...
  )
{
...

  {
    ...
    std::set<gspc::scheduler::job_id_t> const jobs_that_can_be_started {job_0, job_1, job_2};
    F::_scheduler->start_pending_jobs
...

  {
    ...
    BOOST_REQUIRE (  assignment.at (job_0) == std::set<gspc::scheduler::worker_id_t>({"worker_1"})
                  || assignment.at (job_1) == std::set<gspc::scheduler::worker_id_t>({"worker_1"})
                  || assignment.at (job_2) == std::set<gspc::scheduler::worker_id_t>({"worker_1"})
                  );

    // only the worker 1 should get a job started, as worker 0 is still reserved
    // (until the submitted job finishes)
    std::set<gspc::scheduler::job_id_t> const jobs_that_can_be_started {job_0, job_1, job_2};
    std::set<gspc::scheduler::job_id_t> jobs_started;
    F::_scheduler->start_pending_jobs
...
struct fixture_add_new_workers
{
  ...
  std::vector<gspc::scheduler::worker_id_t> add_new_workers
    ( std::unordered_set<std::string> const& cpbnames
    , unsigned int n
    )
  {
    std::vector<gspc::scheduler::worker_id_t> new_workers (n);
    ...
    for (gspc::scheduler::worker_id_t const& worker : new_workers)
    {
  ...
  void add_new_jobs
    ( std::vector<gspc::scheduler::job_id_t>& a
    , std::optional<std::string> reqname
    , unsigned int n
    )
  {
  ...
  std::set<gspc::scheduler::worker_id_t> get_workers_with_assigned_jobs
    (std::vector<gspc::scheduler::job_id_t> const& jobs)
  {
    auto const assignment (get_current_assignment());
    ...
    std::set<gspc::scheduler::worker_id_t> assigned_workers;
  ...
  std::set<gspc::scheduler::job_id_t> get_jobs_assigned_to_worker
    ( gspc::scheduler::worker_id_t const& worker
    , std::map<gspc::scheduler::job_id_t, std::set<gspc::scheduler::worker_id_t>>const& assignment
    )
  {
    BOOST_REQUIRE (!assignment.empty());

    std::set<gspc::scheduler::job_id_t> assigned_jobs;
  ...
  void check_work_stealing
    ( std::vector<gspc::scheduler::worker_id_t> const& initial_workers
    , std::vector<gspc::scheduler::worker_id_t> const& new_workers
    , std::map<gspc::scheduler::job_id_t, std::set<gspc::scheduler::worker_id_t>> old_assignment
    ...
    )
  {
    ...
    for (auto worker : initial_workers)
    {
  ...
  void require_worker_and_implementation
    ( gspc::scheduler::job_id_t const& job
    , std::set<gspc::scheduler::worker_id_t>& expected_workers
    , gspc::scheduler::daemon::Implementation const& impl
    )
  {
    auto const assignment (get_current_assignment());
    BOOST_REQUIRE (assignment.count (job));
...
MULTIPLE_FIXTURES_TEST_CASE
  ...
  , F
  , fixture_add_new_workers <gspc::scheduler::daemon::SingleAllocationScheduler>
  , fixture_add_new_workers <gspc::scheduler::daemon::CoallocationScheduler>
  , fixture_add_new_workers <gspc::scheduler::daemon::GreedyScheduler>
  )
{
  ...
  gspc::testing::unique_random<std::string> gen_capabilities;
  ...
  const std::vector<gspc::scheduler::worker_id_t> initial_workers
    (F::add_new_workers (capabilities, n));

  std::vector<gspc::scheduler::job_id_t> jobs;
  F::add_new_jobs (jobs, std::nullopt, 2*n);

  BOOST_REQUIRE_EQUAL (F::get_workers_with_assigned_jobs (jobs).size(), n);

  F::add_new_jobs (jobs, std::nullopt, 2*k);

  auto const old_assignment (F::get_current_assignment());

  const std::vector<gspc::scheduler::worker_id_t> new_workers
...
{
  std::vector<gspc::scheduler::worker_id_t> workers;
  std::vector<gspc::scheduler::job_id_t> jobs;
  ...
  std::vector<gspc::scheduler::worker_id_t> initial_calc_workers
...

  {
    ...
    const std::vector<gspc::scheduler::worker_id_t> new_load_workers
...

  {
    ...
    const std::vector<gspc::scheduler::worker_id_t> new_reduce_workers
...

  {
    ...
    const std::vector<gspc::scheduler::worker_id_t> new_calc_workers
      (F::add_new_workers ({"CALC"}, n_calc_workers));
  ...
  )
{
  ...
  const std::vector<gspc::scheduler::worker_id_t> workers
    (F::add_new_workers (capabilities, n_workers));

  std::vector<gspc::scheduler::job_id_t> jobs;
  F::add_new_jobs (jobs, std::nullopt, n_jobs);

  BOOST_REQUIRE_EQUAL (F::get_workers_with_assigned_jobs (jobs).size(), n_workers);

  auto const old_assignment (F::get_current_assignment());

  gspc::scheduler::worker_id_t worker_with_2_jobs;
  gspc::scheduler::worker_id_t worker_with_1_job;

  for (gspc::scheduler::worker_id_t const& worker : workers)
  {
  ...
  const std::set<gspc::scheduler::job_id_t> worker_jobs
  ...
  )
{
  ...
  const gspc::scheduler::worker_id_t worker_0
    (F::add_new_workers ({"A", "B"}, 1).at (0));

  const gspc::scheduler::worker_id_t worker_1
  ...
  const gspc::scheduler::worker_id_t worker_with_1_job
    {(n_assigned_jobs_worker_0 == 1) ? worker_0 : worker_1};
  const gspc::scheduler::worker_id_t worker_with_2_jobs
  ...
  )
{
  ...
  std::vector<gspc::scheduler::worker_id_t> workers_with_1_job;
  std::copy_if ( workers.rbegin()
               , workers.rend()
               , back_inserter (workers_with_1_job)
               , [&worker_with_2_jobs] (gspc::scheduler::worker_id_t const& worker)
                 {
  ...
  for (gspc::scheduler::worker_id_t const& worker : workers_with_1_job)
  {
    const std::set<gspc::scheduler::job_id_t> worker_jobs
  ...
  )
{
  ...
  gspc::scheduler::worker_id_t const worker_0 (utils::random_peer_name());
  auto const first_pref (preferences.begin());

  add_worker ( F::_worker_manager
             , worker_0
             , { gspc::scheduler::Capability (common_capability)
               , gspc::scheduler::Capability (*first_pref)
               }
             );

  gspc::scheduler::worker_id_t const worker_1 (utils::random_peer_name());
  add_worker ( F::_worker_manager
             , worker_1
             , { gspc::scheduler::Capability (common_capability)
               , gspc::scheduler::Capability (*std::next (first_pref, 1))
               }
             );

  gspc::scheduler::worker_id_t const worker_2 (utils::random_peer_name());
  add_worker ( F::_worker_manager
             , worker_2
             , { gspc::scheduler::Capability (common_capability)
               , gspc::scheduler::Capability (*std::next (first_pref, 2))
               }
             );
  ...
  )
{
  ...
  auto const preference_id
    (gspc::testing::random<std::size_t>{} (preferences.size() - 1));
  auto const preference (*gspc::util::next (first_pref, preference_id));

  add_worker ( F::_worker_manager
             , worker
             , { gspc::scheduler::Capability (capability)
               , gspc::scheduler::Capability (preference)
               }
             );
  ...
  F::_scheduler->start_pending_jobs
    ( [this]
      ( gspc::scheduler::daemon::WorkerSet const& assigned_workers
      , gspc::scheduler::daemon::Implementation const& implementation
      , gspc::scheduler::job_id_t const& job_
      , std::function<gspc::com::p2p::address_t (gspc::scheduler::worker_id_t const&)>
      )
      {
        BOOST_REQUIRE_EQUAL
  ...
  )
{
  ...
  std::vector<gspc::scheduler::worker_id_t> const cpu_workers
    (F::add_new_workers ( {common_capability, CPU}
                        , num_cpu_workers
                        )
    );

  std::set<gspc::scheduler::worker_id_t> expected_cpu_workers
    (cpu_workers.begin(), cpu_workers.end());

  auto const num_cpu_gpu_workers
    (gspc::testing::random<unsigned int>{} (20, 10));

  std::vector<gspc::scheduler::worker_id_t> const cpu_gpu_workers
    (F::add_new_workers ( {common_capability, CPU, GPU}
                        , num_cpu_gpu_workers
                        )
    );

  std::set<gspc::scheduler::worker_id_t> expected_cpu_gpu_workers
  ...
  )
{
  ...
  gspc::we::type::Preferences preferences;

  unsigned int total_num_workers (0);
  auto const num_preferences
    (gspc::testing::random<unsigned int>{} (12, 3));

  std::map<std::string, std::set<gspc::scheduler::worker_id_t>> workers_by_preference;
  ...
  for (unsigned int i (0); i < num_preferences; ++i)
  {
    ...
    auto const num_workers
      (gspc::testing::random<unsigned int>{} (200, 100));
    total_num_workers += num_workers;

    std::vector<gspc::scheduler::worker_id_t> const workers
    ...
    workers_by_preference.emplace
      (preference, std::set<gspc::scheduler::worker_id_t> (workers.begin(), workers.end()));
  }
  ...
  )
{
  ...
  F::require_worker_and_implementation (job_0, worker_1, std::nullopt);
  ...
  auto const job_3
   ...
   F::require_worker_and_implementation (job_3, worker_1, std::nullopt);
}
  ...
  )
{
  ...
  gspc::scheduler::worker_id_t const worker (utils::random_peer_name());
  add_worker
    (F::_worker_manager, worker, {gspc::scheduler::Capability (capability_pool())});
  ...
  )
{
  ...
  auto const num_workers (gspc::testing::random<std::size_t>{} (100, 10));
  auto const num_tasks
    ( gspc::testing::random<std::size_t>{} (10, 2)
    * num_workers
    );

  std::vector<gspc::scheduler::worker_id_t> test_workers;
  ...
  )
{
  ...
  for (unsigned int k (0); k < num_workers; ++k)
  {
    ...
    add_worker ( F::_worker_manager
               , worker
               , { gspc::scheduler::Capability (common_capability)
                 , gspc::scheduler::Capability
                     (*gspc::util::next (preferences.begin(), k % preferences.size()))
                 }
               );
  }
  ...
  )
{
  ...
  for (unsigned int k (0); k < num_workers; ++k)
  {
    ...
    test_workers.emplace_back (worker);
    add_worker
      (F::_worker_manager, worker, {gspc::scheduler::Capability (common_capability)});
  }
  ...
  auto const worker_finishing_tasks_earlier
    (test_workers[gspc::testing::random<std::size_t>{} (num_workers - 1)]);
