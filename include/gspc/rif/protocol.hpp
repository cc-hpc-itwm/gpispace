...
#include <unistd.h>
    ...
    namespace gspc::rif::protocol
    {
      ...
      struct start_scheduler_result
      {
        pid_t pid;
        hostinfo_t hostinfo;
        gspc::logging::endpoint logger_registration_endpoint;
      };
      ...
      FHG_RPC_FUNCTION_DESCRIPTION
        ...
        , start_scheduler_result
            ( std::string name
            , std::optional<unsigned short> agent_port
      ...
      {
        pid_t pid;
        gspc::logging::endpoint sink_endpoint;
      };

      FHG_RPC_FUNCTION_DESCRIPTION
        ( start_logging_demultiplexer
        , start_logging_demultiplexer_result (std::filesystem::path exe)
        );

      FHG_RPC_FUNCTION_DESCRIPTION
        ( add_emitter_to_logging_demultiplexer
        , void (pid_t, std::vector<gspc::logging::endpoint>)
        );

      FHG_RPC_FUNCTION_DESCRIPTION
        ( start_vmem
        , pid_t ( std::filesystem::path command
  ...
  namespace boost::serialization
  {
        ...
        )
    {
      ...
      for (auto const& x : t)
      {
        std::string const exception_string
          (gspc::util::serialization::exception::serialize (x.second));
        ar << x.first;
        ar << exception_string;
      }
    }
        ...
        )
    {
      ...
      while (size --> 0)
      {
        ...
        t.emplace
          ( pid
          , gspc::util::serialization::exception::deserialize (exception_string)
          );
      }
    }
