...
#include <unordered_map>
      ...
      namespace gspc::util::serialization::exception
      {
        ...
        {
          //! \note needs to either return std::nullopt; or return a
          //! string which can be used to reconstruct a user_exception.
          //! \note one function may handle multiple exception types.
          std::function<std::optional<std::string> (std::exception_ptr)> from_ptr;
        ...
        //! std::nullopt will have the deserialization functions of
...
#include <gspc/util/serialization/exception.ipp>
