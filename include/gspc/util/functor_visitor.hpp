...
#include <variant>
  ...
  namespace gspc::util
  {
    ...
    // Helper to detect if something is a std::variant
    template<typename T>
    ...
    template<typename... Ts>
      struct is_std_variant<std::variant<Ts...> const&> : std::true_type {};

    // For std::variant
    template<typename Ret, typename Variant, typename Visitor>
      std::enable_if_t<is_std_variant<Variant>::value, Ret>
      visit (Variant&& variant, Visitor&& visitor)
    {
