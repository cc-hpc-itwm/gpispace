  ...
  namespace gspc::util
  {
    ...
    public:
      join_reference ( C const& container
                     ...
                     , std::string const& open = ""
                     , std::string const& close = ""
                     , std::optional<size_type> const& max_elements_to_print
                     = std::nullopt
                     )
      ...
      std::ostream& operator() (std::ostream& os) const override
      {
        ...
        return os
          ...
          , _open
          , ( (  !!_max_elements_to_print
              && number_of_elements > _max_elements_to_print.value()
              )
            ? "..." + _close
            : _close
            )
    ...
    private:
      ...
      std::string const _open;
      std::string const _close;
      std::optional<size_type> _max_elements_to_print;
    };
    ...
    template<typename Container, typename Separator>
      constexpr join_reference<Container, Separator>
        join ( Container const& c
             ...
             , std::string const& open
             , std::string const& close
             , std::optional<Difference<Container>> const& max_elements_to_print
             )
    {
