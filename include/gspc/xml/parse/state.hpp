...
#include <filesystem>
    ...
    namespace gspc::xml::parse::state
    {
      ...
      {
      private:
        using in_progress_type = std::list<std::filesystem::path>;
      ...
      public:
        ...
        const ::gspc::xml::parse::type::requirements_type& requirements() const;

        void set_requirement
          (const ::gspc::xml::parse::type::require_key_type& key);

        ::gspc::we::type::property::path_type& prop_path();

        void interpret_property ( ::gspc::we::type::property::path_type const& path
        ...
        std::filesystem::path file_in_progress() const;
        void set_in_progress_position (const char*);
        util::position_type position (const xml_node_type*) const;

        std::set<std::filesystem::path> const& dependencies() const;
...
#undef WARN
        ...
        void check_for_include_loop (std::filesystem::path const& path) const;
        ...
        {
          const std::filesystem::path path (expand (file));
        ...
        std::filesystem::path strip_path_prefix
          (std::filesystem::path const&) const;

      private:
        ::gspc::xml::parse::type::requirements_type _requirements{};
        ...
        std::set<std::filesystem::path> _dependencies{};
        ::gspc::we::type::property::path_type _prop_path{};
        ...
        std::filesystem::path expand (std::string const& file) const;
      };
