...
#include <gspc/xml/parse/type/memory_buffer.fwd.hpp>
#include <gspc/xml/parse/type/mod.fwd.hpp>
#include <gspc/xml/parse/type/net.fwd.hpp>
#include <gspc/xml/parse/type/place.fwd.hpp>
#include <gspc/xml/parse/type/place_map.fwd.hpp>
#include <gspc/xml/parse/type/port.fwd.hpp>
#include <gspc/xml/parse/type/response.fwd.hpp>
#include <gspc/xml/parse/type/specialize.fwd.hpp>
#include <gspc/xml/parse/type/struct.fwd.hpp>
...
#include <optional>
    ...
    namespace gspc::xml::parse::error
    {
     ...
     public:
      ...
      {
      public:
        no_elements_given (std::string const&, std::filesystem::path const&);
      };
      ...
      {
      public:
        explicit file_already_there (std::filesystem::path const& file)
          : generic
      ...
      public:
        include_loop ( std::string const& pre
          ...
          : generic (pre, "include loop: " + gspc::util::join (pos, end, " -> ").string())
        {}
      };
      ...
      class shared_place_does_not_exist : public generic
      {
      public:
        shared_place_does_not_exist
          ( type::place_type const& place
          , std::string const& cleanup_place_name
          );
      };

      // ******************************************************************* //

      class shared_place_not_marked_as_sink : public generic
      {
      public:
        shared_place_not_marked_as_sink
          ( type::place_type const& place
          , std::string const& cleanup_place_name
          , type::place_type const& cleanup_place
          );
      };

      // ******************************************************************* //

      class generator_place_with_tokens : public generic
      {
      public:
        generator_place_with_tokens (type::place_type const& place);
      };

      // ******************************************************************* //

      class generator_place_with_put_token : public generic
      {
      public:
        generator_place_with_put_token (type::place_type const& place);
      };

      // ******************************************************************* //

      class generator_place_with_shared_sink : public generic
      {
      public:
        generator_place_with_shared_sink (type::place_type const& place);
      };

      // ******************************************************************* //

      class generator_place_invalid_type : public generic
      {
      public:
        generator_place_invalid_type (type::place_type const& place);
      };

      // ******************************************************************* //

      class generator_place_with_read_connection : public generic
      {
      public:
        generator_place_with_read_connection
          ( type::place_type const& place
      ...
      class generator_place_with_incoming_connection : public generic
      {
      public:
        generator_place_with_incoming_connection
          ( type::place_type const& place
          , type::transition_type const& transition
          );
      };
...
#undef DUPLICATE
      ...
      public:
        eureka_group_attribute_and_tag
          ...
          , ::gspc::we::type::eureka_id_type const& id_attribute
          , util::position_type const& pod_attribute
          , ::gspc::we::type::eureka_id_type const& id_tag
      ...
      {
      public:
        port_with_unknown_type ( ::gspc::we::type::PortDirection const& direction
      ...
      public:
        port_tunneled_type_error ( std::string const& name_virtual
                                 , gspc::pnet::type::signature::signature_type const& sig_virtual
                                 , std::string const& name_real
                                 , gspc::pnet::type::signature::signature_type const& sig_real
                                 , std::filesystem::path const& path
                                 )
          : generic
            { fmt::format
              ...
              , name_virtual
              , gspc::pnet::type::signature::show (sig_virtual)
              , name_real
              , gspc::pnet::type::signature::show (sig_real)
              )
            , path
            }
        {}
      ...
      class output_not_connected : public generic
      {
      public:
        output_not_connected ( type::transition_type const&
      ...
      class contradicting_shared_attributes : public generic
      {
      public:
        contradicting_shared_attributes (type::transition_type const&);
      };
      ...
      public:
        property_generic ( std::string const& msg
                         , ::gspc::we::type::property::path_type const& key
                         , std::filesystem::path const& path
                         )
          : generic
...
{2}
      ...
      {
      public:
        could_not_open_file (std::filesystem::path const& file)
      ...
      {
      public:
        could_not_create_directory (std::filesystem::path const& path)
      ...
      {
      public:
        template_without_function ( std::optional<std::string> const& name
                                  , std::filesystem::path const& path
                                  )
          : generic
