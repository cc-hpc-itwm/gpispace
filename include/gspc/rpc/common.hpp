...
#include <gspc/util/hash/asio/ip/tcp/endpoint.hpp>
#include <gspc/util/serialization/asio/ip/tcp/endpoint.hpp>
...
#include <utility>
  ...
  //!
  //! The underlying transport layers use Asio so both service
  //! provider and caller need to have a \c ::asio::io_context
  ...
  //! provides \c gspc::util::scoped_boost_asio_io_service_with_threads.
  //!
  ...
  //!
  //!- A caller has
  //!  - a \c ::asio::io_context with threads
  ...
  //!
  //!- A provider has
  //!  - an \c ::asio::io_context with threads
  ...
  //!    provided
    ...
    namespace gspc::rpc::error
    {
      ...
      using namespace gspc::util::serialization;
