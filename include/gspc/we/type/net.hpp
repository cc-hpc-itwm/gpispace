// Copyright (C) 2009-2010,2012-2016,2019-2026 Fraunhofer ITWM
...
namespace gspc::we
{
  namespace edge
  {
    ...
    using type = std::variant<PT, PT_READ, PT_NUMBER_OF_TOKENS, TP, TP_MANY>;
  ...
  namespace type
  {
    ...
    public:
      ...
      transition_id_type add_transition (Transition const&);
      ...
      // Place a value without tracking shared references.
      // Used by wrapper nets which don't contain cleanup places.
      void put_value_without_tracking
    ...
    private:
      ...
      std::unordered_map<transition_id_type, Transition> _tmap;
      ...
      Activity extract_activity
        (transition_id_type, Transition const&);
      ...
      struct SharedSinkInfo
      {
        using ReferenceCounterByValue
          = std::unordered_map< pnet::type::value::value_type
                              , std::size_t
                              , ValueHash
                              >
          ;

        place_id_type _place_id_of_cleanup_place;
        ReferenceCounterByValue _counters;

        template<typename Archive>
          void serialize (Archive& ar, unsigned int)
        {
          ar & _place_id_of_cleanup_place;
          ar & _counters;
        }
      };
      using CleanupPlace = std::string;
      using SharedSinkInfos = std::unordered_map<CleanupPlace, SharedSinkInfo>;
      SharedSinkInfos _shared_sink_infos;

      // Throws when the place is not a cleanup place.
      //
      [[nodiscard]] auto sink_info (CleanupPlace const&) -> SharedSinkInfo&;
      void increase_reference_counter (pnet::type::value::value_type const&);
      using SharedToCheckForCleanup
        = std::unordered_map
          < CleanupPlace
          , std::unordered_set<pnet::type::value::value_type, ValueHash>
          >;
      void decrease_reference_counter
        ( pnet::type::value::value_type const&
        , SharedToCheckForCleanup&
        );
      void cleanup_shared_that_have_no_more_references
        ( SharedToCheckForCleanup const&
        );

      // Generator place support
      std::unordered_map<place_id_type, pnet::type::value::value_type>
        _generator_state;
