...
#include <vector>
  ...
  //! Types of expressions are types of `gspc::pnet::type::value::value_types`s
  ...
  namespace gspc::we::expr::type
  {
    ...
    template<typename Type> struct TypesT
    {
      ...
      std::optional<Type> singleton() const; // exactly one type
      bool is_multi() const;                   // more than one type

      Types _types;
    };

    //! \todo remove gspc::we::signature, use this type instead
...
namespace gspc::we::expr
{
  ...
  namespace gspc::we::expr::type
  {
    ...
    std::ostream& operator<< (std::ostream&, Shared const&);
  ...
  namespace gspc::we::expr::type
  {
    ...
    bool operator== (Bigint const&, Bigint const&);
    bool operator== (Shared const&, Shared const&);
